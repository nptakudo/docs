...
| --- | --- |
| CopyExpand  ``` SELECT ST_DISTANCE(     ST_POINT(13.4814, 52.5015),     ST_POINT(-121.8212, 36.8252))   AS distance_in_meters; ```  Show lessSee more  Scroll to top  Expand  ``` +--------------------+ | DISTANCE_IN_METERS | |--------------------| |   9182410.99227821 | +--------------------+ ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT ST_DISTANCE(     ST_GEOMPOINT(13.4814, 52.5015),     ST_GEOMPOINT(-121.8212, 36.8252))   AS distance_in_degrees; ```  Show lessSee more  Scroll to top  Expand  ``` +---------------------+ | DISTANCE_IN_DEGREES | |---------------------| |       136.207708844 | +---------------------+ ```  Show lessSee more  Scroll to top |
...
| --- | --- |
| CopyExpand  ``` SELECT ST_AREA(border) AS area_in_sq_meters   FROM world_countries   WHERE name = 'Germany'; ```  Show lessSee more  Scroll to top  Expand  ``` +-------------------+ | AREA_IN_SQ_METERS | |-------------------| |  356379183635.591 | +-------------------+ ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT ST_AREA(border) as area_in_sq_degrees   FROM world_countries_geom   WHERE name = 'Germany'; ```  Show lessSee more  Scroll to top  Expand  ``` +--------------------+ | AREA_IN_SQ_DEGREES | |--------------------| |       45.930026848 | +--------------------+ ```  Show lessSee more  Scroll to top |
...
| --- | --- |
| CopyExpand  ``` SELECT name FROM world_countries WHERE   ST_INTERSECTS(border,     TO_GEOGRAPHY(       'LINESTRING(13.4814 52.5015, -121.8212 36.8252)'     )); ```  Show lessSee more  Scroll to top  Expand  ``` +--------------------------+ | NAME                     | |--------------------------| |                  Germany | |                  Denmark | |                  Iceland | |                Greenland | |                   Canada | | United States of America | +--------------------------+ ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT name FROM world_countries_geom WHERE   ST_INTERSECTS(border,     TO_GEOMETRY(       'LINESTRING(13.4814 52.5015, -121.8212 36.8252)'     )); ```  Show lessSee more  Scroll to top  Expand  ``` +--------------------------+ | NAME                     | |--------------------------| |                  Germany | |                  Belgium | |              Netherlands | |           United Kingdom | | United States of America | +--------------------------+ ```  Show lessSee more  Scroll to top |
...
[pruning micro-partitions](../user-guide/tables-clustering-micropartitions.html#label-micropartitions-query-pruning) that can’t contain matching
...
![Bounding-box line with two intersecting shapes and two non-intersecting shapes](../_images/geospatial-pruning.png)
