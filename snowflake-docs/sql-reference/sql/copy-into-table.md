...
:   Required for transforming data during loading
    ...
    details about data loading transformations, including examples, see the usage notes in [Transform data during a load](../../user-guide/data-load-transform).
...
:   Definition:
    :   Specifies the scanner and the scanner options that are used for processing unstructured data.
        ...
        * `SCANNER_OPTIONS`: specifies the properties to the custom scanner type. For example, if you specify `document_ai` as the type of `SCANNER`, you must specify the properties of `document_ai`. The following list shows the predefined set of properties for `document_ai`:
        ...
        For more information, see [Load unstructured data with Document AI](../../user-guide/data-load-unstructured-data).
...
:   Definition:
    :   A user-defined mapping between a target table’s existing columns to its METADATA$ columns. This copy option can only be used with the MATCH\_BY\_COLUMN\_NAME copy option. The following list shows the valid input for `METADATA$field`:
        ...
        * METADATA$START\_SCAN\_TIME
        ...
        For more information about metadata columns, see [Query metadata for staged files](../../user-guide/querying-metadata).
...
* Loading from Iceberg-compatible Parquet files using `LOAD_MODE`:
  ...
  + To load the row lineage metadata columns in Parquet files, which are
    `_row_id` and `_last_updated_sequence_number`, you must use the FULL\_INGEST option. The other LOAD\_MODE options aren’t supported.

    - Parquet files that contain row lineage are likely already part of an Iceberg v3 table. Registering Parquet files by using ADD\_FILES\_COPY is
      ...
      tables to Snowflake-managed Iceberg tables without rewriting files is to use the [ALTER ICEBERG TABLE … CONVERT TO MANAGED](alter-iceberg-table-convert-to-managed)
      command.
...
For examples of data loading transformations, see [Transform data during a load](../../user-guide/data-load-transform).
...
[ALTER ICEBERG TABLE … CONVERT TO MANAGED](alter-iceberg-table-convert-to-managed) command.
...
see [Apache Iceberg™ tables: Support for Apache Iceberg™ v3 (Preview)](../../user-guide/tables-iceberg-v3-specification-support).
