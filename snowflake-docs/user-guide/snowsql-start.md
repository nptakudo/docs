...
| --- | --- |
| `-a, --accountname TEXT` | Your [account identifier](gen-conn-config). Honors $SNOWSQL\_ACCOUNT. |
| `-u, --username TEXT` | Username to connect to Snowflake. Honors $SNOWSQL\_USER. |
...
| `--authenticator TEXT` | Authenticator: ‘snowflake’, ‘externalbrowser’ (to use any IdP and a web browser), <https:/>/<okta\_account\_name>.okta.com (to use Okta natively), ‘workload\_idenity’ or ‘oauth’ to authenticate using OAuth. |
...
1. In the `[connections]` section, configure the default connection parameters by removing the comment symbol from any of
   ...
   [Configuring a client, driver, library, or third-party application to connect to Snowflake](gen-conn-config).
...
After configuration, you can evaluate and troubleshoot your network connectivity to Snowflake using [SnowCD](snowcd).
...
2. Add a separate `[connections]` section with a unique name for each named connection.
   ...
   [account identifier](gen-conn-config) `myorganization-myaccount`:
...
[configuration file](snowsql-config.html#label-configuring-snowsql).
...
To use [browser-based SSO authentication](admin-security-fed-auth-use.html#label-browser-based-sso) for SnowSQL, add `--authenticator externalbrowser` to your [SnowSQL connection parameters](#label-snowsql-connection-parameters):
...
For more information about federated authentication/SSO, see [Managing/Using federated authentication](admin-security-fed-auth-use).
...
> :   Required
>
>     Specifies your [account identifier](gen-conn-config). Specify the account identifier in this form:
>     `organization_name-account_name` (for example, `myorganization-myaccount`).
>
>     For instructions on finding the account identifier, see [Configuring a client, driver, library, or third-party application to connect to Snowflake](gen-conn-config).
>
>     This connection parameter can also be set in the [configuration file](snowsql-config.html#label-configuring-snowsql).
>
> Value:
> :   String
>
...
> :   Provided for backward compatibility/internal use
>
>     Specifies the ID for the [region](intro-regions) where your account is located.
>
>     This parameter is no longer used. For more details, see [-a , --accountname](#label-account-param-snowsql) (in this topic).
>
> Value:
> :   N/A
>
...
>     > * `externalbrowser` [uses your web browser](admin-security-fed-auth-use.html#label-browser-based-sso) to authenticate with Okta, AD FS, or any other SAML 2.0-compliant identity provider (IdP) that has been defined for your account.
>     > * `https://<okta_account_name>.okta.com` (i.e. the URL endpoint for Okta) [authenticates through native Okta](admin-security-fed-auth-use.html#label-native-sso-okta) (only supported if your IdP is Okta).
>     > * `oauth` authenticates using OAuth. When OAuth is specified as the authenticator, you must also set the `--token` parameter to specify the OAuth token ([see below](#label-snowsql-additional-connection-parameters-token)).
>
>     For more information, see [Managing/Using federated authentication](admin-security-fed-auth-use) and [Clients, drivers, and connectors](oauth-intro.html#label-oauth-intro-clients-drivers-conns).
>
> Default:
> :   `snowflake`
>
...
>     can set/override, see [SnowSQL configuration options reference](snowsql-config.html#label-snowsql-options).
>
> Value:
> :   String
>
...
>
>     This parameter applies only to customers who have [replication](account-replication-intro) enabled.
>
> Value:
> :   N/A (parameter doesn’t take a value)
>
