...
In addition, by referencing [metadata columns](querying-metadata) in a staged file, a staged data query can return additional information, such as filename and row numbers, about the file.

Snowflake utilizes support for staged data queries to enable [transforming data during loading](data-load-transform).
