...
The following example loads data from files in the named `my_ext_stage` stage created in [Create an S3 stage](data-load-s3-create-stage). Using pattern matching, the statement only loads files whose names start with the string `sales`:
...
The following example loads all files prefixed with `data/files` in your S3 bucket using the named `my_csv_format` file format created in [Preparing to load data](data-load-prepare):
