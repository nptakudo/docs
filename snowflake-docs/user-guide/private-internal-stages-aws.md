...
[![Connect to internal stage using AWS PrivateLink for S3](../_images/internal-stage-private-connect-aws.png)](../_images/internal-stage-private-connect-aws.png)
...
For more information about the AWS regions in which FIPS is enforced, see [Supported cloud regions](intro-regions).

For information about finding the region names for your account, see [Find the cloud-provider’s name of the region for your account](admin-security-privatelink.html#label-find-csp-region-name).
...
* Set the [ENABLE\_INTERNAL\_STAGES\_PRIVATELINK](../sql-reference/parameters.html#label-enable-internal-stages-privatelink) parameter to enable support for connecting to an internal stage over AWS
...
* Update the firewall allow-listing as follows:
  ...
  + If using an outbound firewall, ensure that it allows all the URLs required by Snowflake. For details, see [SnowCD (Connectivity Diagnostic Tool)](snowcd).
...
1. As a Snowflake account administrator, execute the following statements in your Snowflake account and record the value defined by the
   ...
   to right. For more information, see [ENABLE\_INTERNAL\_STAGES\_PRIVATELINK](../sql-reference/parameters.html#label-enable-internal-stages-privatelink) and
...
The [S3\_STAGE\_VPCE\_DNS\_NAME](../sql-reference/parameters.html#label-s3-stage-vpce-dns-name) parameter allows users to associate a Snowflake account with the DNS name
...
* `<region>` is the [cloud region](intro-regions) that hosts your Snowflake account.
