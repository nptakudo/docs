...
url: "https://docs.snowflake.com/en/user-guide/data-load-local-file-system-copy"
---
...
The following example loads data from all files prefixed with `staged` in your user stage using the named `my_csv_format` file format created in [Preparing to load data](data-load-prepare):
...
The following example loads data from all files from the `my_stage` named stage, which was created in [Choosing an internal stage for local files](data-load-local-file-system-create-stage):
