...
[drop the SEARCH OPTIMIZATION property](cost-estimation.html#label-remove-search-optimization-property) and
[add the SEARCH OPTIMIZATION property](enabling.html#label-search-optimization-service-configuration-entire-table) back to the table.
...
If you [drop the SEARCH OPTIMIZATION property](cost-estimation.html#label-remove-search-optimization-property) from the table, the search access
...
also cloned. Cloning a table, schema, or database creates a [zero-copy clone](../tables-storage-considerations.html#label-cloning-tables) of each table
...
2. Disable the search optimization service on the cloned table immediately after the clone is created. For example, to disable
   ...
   For more information, see [Search optimization actions (searchOptimizationAction)](../../sql-reference/sql/alter-table.html#label-alter-table-searchoptimizationaction) in the ALTER TABLE topic.
...
replicated cloned tables. Replication doesn’t create [zero-copy clone](../tables-storage-considerations.html#label-cloning-tables) for cloned search access
...
kinds of costs described in [Search optimization cost estimation and management](cost-estimation.html#label-search-optimization-maintenance-billing).

## Sharing tables

Data providers can use [Secure Data Sharing](../data-sharing-intro) to share tables that have search optimization
enabled.
