...
[point lookups of semi-structured data](../querying-semistructured) where the
...
[path to an element in the column with a semi-structured data type](../querying-semistructured.html#label-traversing-semistructured-data).
...
The search optimization service can optimize [wildcard or regular expression searches](substring-queries.html#label-search-optimization-service-queries-wildcard-regexp)
...
which is described in [Querying Semi-structured Data](../querying-semistructured), see
[Enable FULL\_TEXT search optimization on a VARIANT column](text-queries.html#label-search-optimization-service-queries-text-examples-variant).
