...
url: "https://docs.snowflake.com/en/user-guide/snowflake-postgres/postgres-connection-pooling"
---
...
the `user_lookup` function created by `snowflake_pooler` will deny lookups on superusers and replication roles. See [Snowflake Postgres Roles](postgres-roles)
...
In order to use PgBouncer’s support for protocol-level prepared statements, the PgBouncer [max\_prepared\_statements setting](postgres-server-settings) must be set to a value greater than `0`. The default on Snowflake Postgres is `250`, but you can set
it to a different value if desired.

#### Session

Session pooling mode is supported on Snowflake Postgres if you have a need for it. To use this pooling mode, set the [pool\_mode setting](postgres-server-settings) to `session` on your cluster.
...
set the [pool\_mode setting](postgres-server-settings) to `statement` on your cluster.
