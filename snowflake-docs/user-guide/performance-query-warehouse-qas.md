...
of queries running on a warehouse. For complete details about query acceleration, refer to [Using the Query Acceleration Service (QAS)](query-acceleration-service).

The query acceleration service offloads portions of query processing to
[serverless compute resources](cost-understanding-compute.html#label-serverless-credit-usage), which speeds up the processing of a query while reducing its demand
...
You can use the warehouse’s [scale factor](query-acceleration-service.html#label-query-acceleration-scale-factor) to help control the cost of the query acceleration
