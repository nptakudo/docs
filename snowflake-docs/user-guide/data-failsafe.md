...
![Fail-safe in Continuous Data Protection lifecycle](../_images/cdp-lifecycle-fs.png)
...
* Fail-safe doesn’t support tables that contain data ingested by Snowpipe Streaming Classic. For such tables, you can’t use fail-safe for recovery because fail-safe operations on that table will fail completely. For more information, see [Snowpipe Streaming limitations](snowpipe-streaming/data-load-snowpipe-streaming-overview.html#label-snowpipe-streaming-limitations).
