...
Loading data requires a [warehouse](warehouses). If you are using a warehouse that is
...
The following example loads data from files in the named `my_gcs_stage` stage created in [Configure an integration for Google Cloud Storage](data-load-gcs-config). Using pattern matching, the statement only loads files whose names start with the string `sales`:
...
The following example loads all files with the `data/files` path (i.e. prefix) in your Cloud Storage bucket using the named `my_csv_format` file format created in [Preparing to load data](data-load-prepare). Note that a path can be combined with pattern matching:
