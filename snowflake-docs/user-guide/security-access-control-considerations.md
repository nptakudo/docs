...
role lower in a [role hierarchy](security-access-control-overview.html#label-role-hierarchy-and-privilege-inheritance), has sufficient privileges on the objects.
...
* The security administrator (SECURITYADMIN system-defined) role includes the global MANAGE GRANTS privilege to grant or revoke privileges
  ...
  the children system-defined roles, see [System-defined roles](security-access-control-overview.html#label-access-control-overview-roles-system).
...
For instructions to create a role hierarchy, see [Creating a role hierarchy](security-access-control-configure.html#label-security-role-hierarchy).
...
[![Example: Hierarchy of access and functional roles](../_images/role-hierarchy-practical.png)](../_images/role-hierarchy-practical.png)
...
Database roles are essentially the same as traditional [roles](security-access-control-overview.html#label-access-control-overview-role-types) created at the account
...
:   Database owners can independently manage access to securable objects within their own databases. Database owners can perform the
    ...
    * Create or extend [role hierarchies](security-access-control-overview.html#label-role-hierarchy-and-privilege-inheritance). Grant database roles to other database
      ...
      see [Role hierarchy and privilege inheritance](security-access-control-overview.html#label-role-hierarchy-and-privilege-inheritance).
...
Data Sharing:
:   Data providers in Snowflake’s [Secure Data Sharing](data-sharing-intro) can segment the securable objects in a share
...
Note that database roles cannot be [activated](security-access-control-overview.html#label-access-control-overview-active-roles) directly in a session. Grant database
...
can grant privileges on objects in the schema, including [future grants](security-access-control-configure.html#label-granting-future-privs-on-schema-objects), centralizing
...
For more information on managed access schemas, see [Creating managed access schemas](security-access-control-configure.html#label-managed-access-schemas).
...
For more information on future grants, see [Assigning future grants on objects](security-access-control-configure.html#label-granting-future-privs-on-schema-objects).
...
For more details about cloning, see [Cloning considerations](object-clone) and [CREATE <object> … CLONE](../sql-reference/sql/create-clone).
...
To prevent object owners from indiscriminately granting access to objects, use [managed access schemas](security-access-control-configure.html#label-managed-access-schemas).
...
[Creating custom roles](security-access-control-configure.html#label-security-custom-role), execute the following command:
