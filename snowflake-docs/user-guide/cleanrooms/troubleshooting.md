...
url: "https://docs.snowflake.com/en/user-guide/cleanrooms/troubleshooting"
---
...
If you are a developer, see the [developer troubleshooting guidelines](developer-introduction.html#label-dcr-developer-troubleshooting) for additional information.

## Installation issues

* See the [installation troubleshooting section](installing-dcr.html#label-dcr-troubleshooting-installation).
* Confirm that you have [updated your network policy](admin-tasks.html#label-cleanrooms-configure-network-policy) to allow the UI to access your data.
...
Solution:
:   Ensure that both provider and consumer accounts have [enabled external and Iceberg tables](register-data.html#label-cleanrooms-external-iceberg-tables).
...
If you are having problems with an external data connector for AWS, Azure, or Google Cloud Storage, see [Snowflake Data Clean Rooms: Troubleshooting external data connectors](external-data-troubleshoot).
...
Solution:
:   Confirm that your email was validated by clean rooms, and that you fulfill all the [clean rooms account requirements](installing-dcr.html#label-cleanroom-account-requirements).
...
* If this is an external or Iceberg table, confirm that you have fulfilled the [requirements and procedure](register-data.html#label-cleanrooms-external-iceberg-tables) for registering an external or Iceberg table.
* Confirm that your current role has OWNERSHIP privilege on the object being registered.
...
Solution:
:   See [Enabling external and Apache Iceberg™ tables](register-data.html#label-cleanrooms-external-iceberg-tables) to ensure that you fulfill the requirements and procedures to use these table types. You can sometimes resolve this by explicitly granting SELECT on the table to SAMOOHA\_BY\_SNOWFLAKE.
...
:   Tell the provider that they should install the clean rooms environment in their account by following the instructions here:
    [Installing the Snowflake Data Clean Rooms environment](installing-dcr). After that, the provider can re-create and share the clean room.
...
Solution:
:   The provider must complete the process of [enabling external and Iceberg tables for their account](register-data.html#label-cleanrooms-external-iceberg-tables). If they are doing this in code, the provider must check the security scan results and, if successful, must update the default release version.
...
Solution:
:   [Read the external and Iceberg table registration information](register-data.html#label-cleanrooms-external-iceberg-tables) and be sure to follow all instructions on both the provider and consumer side.
