...
* **The clean rooms UI:** Use the clean rooms UI to configure, manage, or create clean rooms. Most clean room analysts use the UI rather
  ...
  [features that are available only in the clean rooms UI](getting-started.html#label-dcr-web-app-api-comparison).
* **Snowsight** is useful to explore databases and other objects and search for objects.
* **Clean rooms API:** API documentation is divided into <provider> and
  <consumer> topic pages.
...
[grant you full API access](manage-dcr-users.html#label-cleanrooms-add-developers). Clean rooms also supports
[creating roles with access to a subset of API procedures](manage-dcr-users.html#label-dcr-grant-limited-access).

You must use the clean rooms API in a warehouse that SAMOOHA\_APP\_ROLE can use. `app_wh` is one of a
[number of warehouses](installation-details.html#label-cleanrooms-installation-details-warehouses) with access to the API. Choose the appropriate warehouse
for your needs.
...
[any other supported Snowflake language](../../developer-guide/stored-procedure/stored-procedures-overview.html#label-stored-procedures-handler-languages).

Procedures exist inside the following schemas:

* `samooha_by_snowflake_local_db.provider` - [Provider-specific procedures](provider). These procedures can be
  called only on clean rooms that were created in the current account.
* `samooha_by_snowflake_local_db.consumer` - [Consumer-specific procedures](consumer). These procedures can be
...
However, a few features are [available only in the UI](getting-started.html#label-dcr-web-app-api-comparison), and some are
...
[cross-cloud behavior](enabling-laf).
...
installed with a clean room package in [Snowflake Data Clean Rooms: Installed objects](installation-details).

### Sample data

The clean rooms environment installs [a few sample datasets](installation-details.html#label-dcr-samooha-sample-database) you can use.

You can also [generate synthetic test data](../synthetic-data) using Snowflake.
...
* The clean room was created in a different cloud hosting region and you haven’t enabled
  [cross-cloud auto-fulfillment](enabling-laf).
...
:   Test this by running the following SQL code:
    ...
    If the code snippet returns TRUE, you have limited-access [run-role](manage-dcr-users.html#label-dcr-grant-limited-access) permissions on the clean
    ...
    the [consumer.grant\_run\_on\_cleanrooms\_to\_role documentation](consumer.html#label-dcr-sproc-consumer-manage-role-access).
