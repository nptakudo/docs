...
in your Snowflake account, see [Installing the Snowflake Data Clean Rooms environment](installing-dcr).
...
significant new releases in the [feature updates section](../../release-notes/new-features.html#label-new-features-features) of the Snowflake release notes page (search
for “clean rooms”).
...
Clean rooms come with [several warehouses](installation-details.html#label-cleanrooms-installation-details-warehouses) that can access the API. Choose the
...
[provider activation](provider.html#label-dcr-api-provider-update-activation-warehouse).
...
user that was created when the [Snowflake account was configured](installing-dcr.html#label-dcr-install-clean-rooms-ui-step).
...
[migrate to Snowflake authentication](update-to-oauth) before requesting SSO.
...
[key-pair authentication](../key-pair-auth) to authenticate. If your Snowflake account uses [authentication policies](../authentication-policies)
...
1. Open Snowflake Admin » Service User Management and select [![Edit icon](../../_images/edit_icon_material_symbol.svg)](../../_images/edit_icon_material_symbol.svg) (Edit).
2. Change the name of the service user to a user that you have [created](installing-dcr.html#label-dcr-create-service-user-manually) and is accessible in
   the current account.
...
Clean rooms doesn’t support key rotation using [RSA\_PUBLIC\_KEY\_2](../key-pair-auth.html#label-key-pair-rotation), so ignore the information about RSA\_PUBLIC\_KEY\_2.
...
read the [activation instructions](activation).
...
1. [Sign in to the clean room environment in the clean rooms UI](web-app-introduction.html#label-cleanroom-web-app-sign-in) as a DCR administrator.
...
3. In the Activation section, select Edit.
   ...
   * To manage **third-party activation**: Check or clear the checkbox next to the third-party activation target you wish to enable or
     ...
     [See the list of available third party connectors](connector-activation).

[Learn how to implement activation in a clean room.](activation)

## Configure network policies

If your Snowflake account uses a [network policy](../network-policies) to control network traffic, you must explicitly allow
traffic from the IP addresses that the clean rooms UI uses to communicate with your Snowflake account.

Find the IP addresses used for your region in the **IP network addresses used by clean rooms UI** column in the [IP address table](web-app-introduction.html#label-web-app-hosting).
