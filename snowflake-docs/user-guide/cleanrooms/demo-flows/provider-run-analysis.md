...
![Basic data flow direction in a provider-run analysis](../../../_images/provider-run-analysis.svg)
...
For more information, see [Snowflake Data Clean Rooms: Installed objects](../installation-details).
...
* Providers can activate results to their own account using the UI or the API, or to third-party providers if using the UI. For information
  about how to enable activation and view results, see [Activating query results](../activation).
* If the consumer and provider are in different cloud regions, [Cross-cloud auto-fulfillment](../enabling-laf)
...
1. The provider [creates and configures a clean room](../manage-clean-rooms.html#label-dcr-create-cleanroom), using one of the
...
3. The consumer [joins and configures the clean room](../manage-clean-rooms.html#label-cleanrooms-web-app-install) as usual for all templates in the clean
   ...
   * The consumer can run queries as soon as the clean room is joined, but there is a delay of up
     ...
     template.
4. After the join step completes, the clean room is available for both [provider run analyses](../web-app-working.html#label-web-app-provider-run) and
   [consumer run analyses](../web-app-working.html#label-web-app-working-run-analysis-consumer).
...
2. **Consumer**
   ...
   2. Sets any [join and column policies](../policies) needed on their data.
...
3. **Provider**
   ...
   1. After the consumer has installed the clean room, the provider enables analyses to access consumer data by enabling data sharing
      ...
      * If the provider and consumer are in **different cloud regions**, the provider and consumer must enable
        [cross-cloud auto-fulfillment](../enabling-laf). When a provider runs an analysis across regions,
