...
url: "https://docs.snowflake.com/en/user-guide/cleanrooms/demo-flows/custom-templates"
---
...
[read the clean room reference guide for custom templates](../custom-templates) before trying to create your
...
You can also upload [custom Python UDFs](custom-code) for your template to call.
...
`provider.submit_analysis_request` for [provider-run analyses](provider-run-analysis).
...
the provider [obfuscated the template](../provider.html#dcr-provider-add-custom-sql-template). Only provider-written templates can be obfuscated.
...
[consumer grants permission](provider-run-analysis).
...
![Rendered user input form with table, age, and color pickers.](../../../_images/custom-template-entry-form.png)
...
tables, and more. For more information about form element types, see [add\_ui\_form\_customizations](../provider.html#dcr-provider-add-ui-form-customizations).
...
reference all columns using their `p` or `c` aliases. [Learn more about aliasing.](../custom-templates.html#label-dcr-required-template-table-aliases)
...
First develop your template and any [scripts](custom-code) that it calls by using only the clean
