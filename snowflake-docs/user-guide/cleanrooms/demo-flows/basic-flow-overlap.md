...
Try out the [web interface tutorial](../tutorials/cleanroom-web-app-tutorial) to see a full end-to-end
...
1. Sign in to the clean rooms UI and [create a new clean room](../manage-clean-rooms.html#label-dcr-create-cleanroom).
...
4. Under Configure Analysis & Query, do the following:
   ...
   >      [Differential Privacy in Snowflake Data Clean Rooms](../differential-privacy) and understand the costs of
...
5. Under Share clean rooms, do the following:
   ...
   * Select Enable run analysis and query next to a collaborator to control whether that account can run a template in the clean room.
     ...
     [provider-run analyses](provider-run-analysis) is disabled by default). By default, consumers
...
1. [Sign in to the clean rooms UI and join the clean room](../manage-clean-rooms.html#label-cleanrooms-web-app-install).
2. Under Add Data, do the following:
...
4. If conducting the analysis as a consumer, you can change the [warehouse size](../../warehouses-overview) to improve query
...
5. Select Run. If this is a new query, do the following:
   ...
   * Optionally [schedule a repeating analysis](../schedule-analysis).
...
The notebook does not demonstrate how to use identity providers, [activation](../activation)
to third-party providers, or [provider-run analyses](provider-run-analysis). See the
