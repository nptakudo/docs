...
url: "https://docs.snowflake.com/en/user-guide/cleanrooms/demo-flows/custom-code"
---
...
[multi-step flow](../multistep-flows). Python is the only coding language supported for custom UDFs.
...
and the [Snowpark API](snowpark.html#label-dcr-snowpark-udf).
...
* [Snowpark Container Services running within a clean room](snowpark.html#label-dcr-snowpark-spcs): If you need more control over the
...
* [How to write UDTFs in Snowflake](../../../developer-guide/udf/python/udf-python-tabular-functions) if you want to return tables from
  your functions.
* [How to create and upload custom templates into a clean room.](custom-templates) UDFs/UDTFs are
  called from a custom template.
* [Using Snowpark in clean rooms](snowpark) (if you want to call your UDFs from Snowpark).
...
these tables can be accessed as described in [Designing multi-step flows](../multistep-flows).
...
If you need a non-default package, you must use [Snowpark Container Services in a clean room](snowpark.html#label-dcr-snowpark-spcs) to host your
code.
...
4. After each successful upload, a [new patch version of the clean room is generated](../dcr-versions). You
...
[Learn more about clean room versioning.](../dcr-versions)
...
3. Upload the .py file to the stage for your clean room. There are
   [several ways to do this](../../data-load-local-file-system-stage), including using the CLI, Snowsight, or
...
Consumer-uploaded code is bundled and uploaded with a custom template using the [consumer template upload flow](custom-templates.html#label-dcr-consumer-written-templates). The uploaded code can be called by any template in the clean room.

To upload code as a consumer, you should understand [custom template syntax](custom-templates).
...
9. The consumer and provider continue with the standard
   [consumer-defined template flow](custom-templates.html#label-dcr-consumer-written-templates):
