...
url: "https://docs.snowflake.com/en/user-guide/cleanrooms/activation"
---
...
* **Third-party activation**, where the provider or consumer pushes results to a Snowflake-approved third-party, such as LiveRamp or Meta
  Ads Manager, through an [activation connector](connector-activation).
...
policies see [Understanding clean room table policies](policies).

Activation supports differential privacy, if enabled, and respects differential privacy rules and budgets.

Important

If the consumer and provider are in different cloud regions, you need to enable [Cross-cloud auto-fulfillment](enabling-laf) in both accounts and for both clean rooms.
...
* [Custom templates](custom-templates.html#label-dcr-custom-templates-activation)

### Supported combinations

Activation can be run either by the provider or the consumer. (Learn more about
[provider-run analyses](demo-flows/provider-run-analysis).)
...
* Activation when using the clean rooms UI requires that the clean rooms account [allows activation](admin-tasks.html#label-cleanrooms-administer-activation).
* For provider-run activation, the clean room must be [configured to support provider-run analysis](demo-flows/provider-run-analysis).
...
>    [custom activation template](custom-templates.html#label-dcr-custom-templates-activation) to the clean room. If this clean room is to be used
...
the provider and consumer, each with the clean rooms API installed. Replace the information as noted in the sample files. [See instructions to upload a SQL worksheet into your Snowflake account](tutorials-and-samples.html#label-dcr-list-of-sample-notebooks-and-worksheets).
...
* The **consumer** must run the following SQL command:
  ...
  where `$provider_account_data_sharing_id` is the provider’s [Data Sharing Account Identifier](../admin-account-identifier.html#label-account-name-data-sharing)
* The **provider** must run the following procedure:
  ...
  where `$consumer_account_data_sharing_id` is the consumer’s [Data Sharing Account Identifier](../admin-account-identifier.html#label-account-name-data-sharing).
...
>    [custom activation template](custom-templates.html#label-dcr-custom-templates-activation) to the clean room. If this clean room is to be used
>    in the clean rooms UI, you must [provide a web form](demo-flows/custom-templates.html#label-dcr-define-user-form) with
...
> SAMOOHA\_LOCAL\_DB\_NAME\_PLACEHOLDER.PUBLIC.CONSUMER\_ACTIVATION\_SUMMARY. Data older than 28 days is expunged from the consumer’s
> account.
...
>    [provider.update\_activation\_warehouse](provider.html#label-dcr-api-provider-update-activation-warehouse) to increase the warehouse
...
[third-party activation connector](connector-activation).

Third-party activation is supported only in the clean rooms UI, and not using custom templates.

Activation when using the clean rooms UI is supported only if the clean rooms account [allows activation](admin-tasks.html#label-cleanrooms-administer-activation).
