...
[Snowflake differential privacy](../diff-privacy/differential-privacy-overview) and
[clean room differential privacy](differential-privacy).
...
* [Query requirements for aggregation policies](../aggregation-policies.html#label-aggregation-policy-query-reqs). For example, you can use this section to
  determine that the MIN and MAX aggregation functions do not satisfy the query requirements, and cannot be used.
* [Aggregation policy limitations](../aggregation-policies.html#label-aggregation-limitations)
...
:   In some circumstances, you can return values without using an aggregation function.
    ...
    | Allowed | Not allowed |
    | --- | --- |
    | CopyExpand  ``` SELECT gender, regions   FROM TABLE sample_db.demo.customer   GROUP BY gender, region; ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT gender, regions   FROM TABLE sample_db.demo.customer; ```  Show lessSee more  Scroll to top |
...
:   | Allowed |
    | --- |
    | CopyExpand  ``` SELECT p.education_level,   c.status,   AVG(p.days_active),   COUNT(DISTINCT p.age_band)   FROM  samooha_sample_database.demo.customers c   INNER JOIN   samooha_sample_database.demo.customers p     ON  c.hashed_email = p.hashed_email   GROUP BY ALL; ```  Show lessSee more  Scroll to top |
...
:   | Allowed |
    | --- |
    | CopyExpand  ``` SELECT COUNT(*),   DATE_TRUNC('week', date_joined) AS week   FROM consumer_sample_database.audience_overlap.customers   GROUP BY week; ```  Show lessSee more  Scroll to top |
...
:   | Allowed |
    | --- |
    | CopyExpand  ``` SELECT COUNT(DISTINCT t1."hashed_email")   FROM provider_sample_database.audience_overlap."customers quoted" t1   INNER JOIN   consumer_sample_database.audience_overlap.customers t2     ON t1."hashed_email" = t2.hashed_email; ```  Show lessSee more  Scroll to top |
