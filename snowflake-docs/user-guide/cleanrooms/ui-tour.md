...
url: "https://docs.snowflake.com/en/user-guide/cleanrooms/ui-tour"
---
...
[upgraded to use Snowflake authentication](update-to-oauth). Additionally, some pages require that you have a
[specific clean rooms role](manage-dcr-users.html#label-dcr-manage-ui-users) to be able to access. If you don’t have the appropriate role to access a
...
To follow along in your browser, [sign in to the clean rooms UI](web-app-introduction.html#label-cleanroom-web-app-sign-in).
...
![Clean rooms list page](../../_images/dcr-clean-rooms-list-page.png)
...
:   Clean rooms are grouped into those you have created, those you have joined (installed) as a consumer, and those you are invited to join as
    ...
    * Created tab: Shows clean rooms that you created by using the UI or the API. Clean rooms created by using the API are labeled as
      ...
      + Edit a clean room you have created by selecting Edit, or [![Three vertical dots indicating more options](../../_images/vertical-more-icon.png)](../../_images/vertical-more-icon.png) » Edit in a clean room tile.
      + If a tile shows a Run button in the Created list, the clean room is enabled for [provider-run analyses](demo-flows/provider-run-analysis).
      + To delete a clean room, select [![Three vertical dots indicating more options](../../_images/vertical-more-icon.png)](../../_images/vertical-more-icon.png) » Delete.
    * Joined tab: Shows clean rooms that you have *joined* (installed as a consumer) using either the UI or the API. After you join a clean
      ...
      values, reset the clean room to original values, or leave (uninstall) the clean room by using the [![Three vertical dots indicating more options](../../_images/vertical-more-icon.png)](../../_images/vertical-more-icon.png) (More) list
      options. For more information, see [Install (join) a clean room](manage-clean-rooms.html#label-cleanrooms-web-app-install) or [Delete a clean room that you created](manage-clean-rooms.html#label-dcr-single-cleanroom-delete).
...
:   Select this button to create a new clean room. For more information, see [Create a new clean room](manage-clean-rooms.html#label-dcr-create-cleanroom).
...
![Analysis rooms list page](../../_images/dcr-clean-rooms-analysis-list.png)
...
![Analysis results page](../../_images/dcr-clean-rooms-analysis-result.png)
...
analysis, select Activate and [walk through the activation flow](activation) to specify who should get
the full results.
...
[identity providers, data providers,](connector-identity) and [activation providers](connector-activation).
...
* See and manage the [service user](installing-dcr.html#label-dcr-create-service-user-manually) that is used by the UI to perform clean room actions.
* See and manage account-wide features, such as [Cross-Cloud Auto-Fulfillment](enabling-laf),
  [external and Iceberg tables](register-data.html#label-cleanrooms-external-iceberg-tables), and scheduled repeating analyses.
* [Register data objects](register-data) for use in clean rooms.
