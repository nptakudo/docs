...
url: "https://docs.snowflake.com/en/user-guide/cleanrooms/update-to-oauth"
---
...
2. **Go through each row in the list** and grant proper Snowflake clean room privileges to each user:
   ...
   * For each row, check the `Users on Snowflake` value:
     ...
     > + **If one user is listed,** grant the [appropriate privileges](manage-dcr-users.html#label-dcr-manage-ui-users) to the username in `Users on Snowflake`.
     > + **If multiple users are listed,** grant the [appropriate privileges](manage-dcr-users.html#label-dcr-manage-ui-users) to each user individually.
     ...
     >   [appropriate privileges](manage-dcr-users.html#label-dcr-manage-ui-users) in Snowflake. (Before the change, you could access the clean room UI
   ...
   * If a user is not listed here who should have access to the clean room UI, grant the user the
     [appropriate privileges](manage-dcr-users.html#label-dcr-manage-ui-users) in Snowflake.
...
* You have not verified ownership of the Snowflake email address now associated with your clean room account. If this is the case,
  [verify your email address](../ui-snowsight-profile.html#label-snowsight-verify-email-address) in Snowflake.
