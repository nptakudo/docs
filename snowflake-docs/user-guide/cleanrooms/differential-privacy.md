...
url: "https://docs.snowflake.com/en/user-guide/cleanrooms/differential-privacy"
---
...
math-based privacy system [[1]](#id2) to provide entity-level data protection for both single queries and repeated querying of a data set.
...
[Snowflake differential privacy](../diff-privacy/differential-privacy-admin-privacy-policies), so read this document to
...
[Snowflake privacy policies](../diff-privacy/differential-privacy-admin-privacy-policies) prevent creation of a view from
...
2. Select Edit or [![Three vertical dots indicating more options](../../_images/vertical-more-icon.png)](../../_images/vertical-more-icon.png) » Edit on the clean room tile (depending on whether the clean room allows you to
   run an analysis).
...
[aggregation policy](../aggregation-policies) on the source tables, or enforce aggregation in your templates.

## Managing differential privacy costs

Differential privacy [incurs costs](cleanroom-cost) even when individual users or templates have not enabled
...
2. Finally, suspend the differential privacy background task in your account by calling the
   [provider.suspend\_account\_dp\_task procedure](provider.html#dcr-provider-suspend-account-dp-task) in Snowsight.
...
or call the [provider.resume\_account\_dp\_task procedure](provider.html#dcr-provider-resume-account-dp-task) in Snowsight.
