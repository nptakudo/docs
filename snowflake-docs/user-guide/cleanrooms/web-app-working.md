...
The [clean rooms UI](web-app-introduction) of a Snowflake Data Clean Room provides an intuitive UI that allows business
...
If a provider has [configured a clean room to allow provider-run analyses](demo-flows/provider-run-analysis.html#label-dcr-enable-provider-run-analysis-ui),they can run an analyses in the clean room.
...
Administrators can monitor the analyses being run by the provider: see [Monitor provider-run analyses](admin-tasks.html#label-cleanrooms-admin-monitor-provider-run).

Consumers can monitor billing due to provider-run analyses: see [Billing and cost details](demo-flows/provider-run-analysis.html#label-dcr-provider-run-billing-and-costs).

Providers can [activate results to their own account](activation.html#label-clean-rooms-push-provider-run-analysis).
...
* If the collaborators are in different clouds or regions:
  ...
  + Consumers must [enable cross-cloud auto-fulfillment](enabling-laf.html#label-cleanrooms-get-started-ccaf) on their account.
...
1. [Sign in to your clean room environment in the clean rooms UI](web-app-introduction.html#label-cleanroom-web-app-sign-in).
...
You can select which [warehouse](../warehouses-overview) you want to use to run an analysis. Increasing the
...
analysis. For information about how credit consumption grows as you use a larger warehouse, see [Warehouse size](../warehouses-overview.html#label-warehouse-size) and
[Billing for Snowpark-optimized warehouses](../warehouses-snowpark-optimized.html#label-snowpark-wh-credits).

For a description of the warehouses that are available, see [Warehouses](installation-details.html#label-cleanrooms-installation-details-warehouses).

If you are an administrator who wants to create additional warehouse options, see [Using a different warehouse](admin-tasks.html#label-cleanrooms-admin-add-warehouse).
