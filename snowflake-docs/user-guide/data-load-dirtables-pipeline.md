...
A [stream](streams-intro) records data manipulation language (DML) changes made to a directory table,
table, external table, or the underlying tables in a view. A [task](tasks-intro) executes a single action,
...
[![A simple data processing pipeline that uses a stream to track changes to a directory table.](../_images/data-lake-dirtable-pipeline.png)](../_images/data-lake-dirtable-pipeline.png)
...
[enable unstructured data access on the stage](unstructured-intro.html#label-file-url-server-side-encryption).
...
see [Reading a dynamically-specified file with SnowflakeFile](../developer-guide/udf/python/udf-python-examples.html#label-reading-file-from-python-udf-snowflakefile).
...
To upload files with Snowsight, see [Upload files onto a named internal stage](data-load-local-file-system-stage-ui.html#label-snowsight-stage-upload-files-internal).
