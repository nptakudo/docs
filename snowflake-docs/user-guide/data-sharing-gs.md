...
Are you a consumer interested in consuming shared data? See [Consume imported data](data-share-consumers).
...
* **Grant a database role to a share**
  ...
  [account roles](security-access-control-overview.html#label-access-control-overview-role-types) in their own account.
  ...
  see [Share data from multiple databases](data-sharing-multiple-db).
...
[![Data sharing with database roles example](../_images/database-role-hierarchy-example1.png)](../_images/database-role-hierarchy-example1.png)

For more information about the privileges, see [Access control privileges](security-access-control-privileges).
...
* To perform the tasks described in this topic, you must use the ACCOUNTADMIN role or a
  [role granted the relevant privileges](security-access-privileges-shares). For more information, including
  additional data sharing scenarios, see [Create and configure shares](data-sharing-provider).
...
[cloud platforms](intro-cloud-platforms), note that currently, replicating a primary database is blocked if one
...
For more information, see [Streams on shared objects](data-sharing-provider.html#label-data-sharing-streams).
...
For more details, refer to [Use secure objects to control data access](data-sharing-secure-views).
...
other data provider tasks, refer to [Create and configure shares](data-sharing-provider).
