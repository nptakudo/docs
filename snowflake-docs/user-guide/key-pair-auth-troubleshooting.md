...
[key pair authentication](key-pair-auth). It focuses on errors that contain `JWT token is invalid`.
...
| 390144 | JWT\_TOKEN\_INVALID | There is a general issue with the JWT token. For possible solutions, see [Common Errors and Solutions](#label-key-pair-troubleshooting-common-errors). |
| 394300 | JWT\_TOKEN\_INVALID\_USER\_IN\_ISSUER | The user name specified in the issuer does not exist in the Snowflake account. For possible solutions, see [Common Errors and Solutions](#label-key-pair-troubleshooting-common-errors). |
...
| 394302 | JWT\_TOKEN\_INVALID\_ISSUE\_TIME | The JWT token was received by Snowflake more than 60 seconds after the issue time. For possible solutions, see [Common Errors and Solutions](#label-key-pair-troubleshooting-common-errors). |
| 394303 | JWT\_TOKEN\_INVALID\_EXPIRATION\_TIME | The JWT token is expired. |
| 394304 | JWT\_TOKEN\_INVALID\_PUBLIC\_KEY\_FINGERPRINT\_MISMATCH | There is a mismatch between the public key fingerprint specified in the issuer and the one stored for the user in Snowflake. For possible solutions, see [Common Errors and Solutions](#label-key-pair-troubleshooting-common-errors). |
...
:   Double-check that the client (driver, connector, or request URL) is using the correct
    [account identifier](admin-account-identifier.html#label-account-name) to connect to the Snowflake account. You should also check that this value matches the
