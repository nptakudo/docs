...
2. [Create one or more network policies](#label-network-policy-create-policy) that include the network rules that contain the identifiers to be allowed or blocked.
3. [Activate the network policy for an account, user, or security integration](#label-associating-network-policies). A network policy does not restrict network traffic until it is activated.
...
[Private connectivity](private-connectivity-inbound) network rules — that is, network rules of type AWSVPCEID or
...
:   Network policies applied to a security are the most specific network policies. They override both accounts and users.
    ...
    authorization server. For more information, see [Restricting network traffic for Snowflake OAuth](oauth-snowflake-overview.html#label-snow-oauth-network-policies).
...
policy, the administrator can organize these restrictions using [network rules](network-rules), which are schema-level
objects.
...
For a complete list of the types of identifiers that can be restricted using network rules, see [Supported network identifiers](network-rules.html#label-network-rule-identifiers).
...
You can then use the `TYPE` property to specify the [identifiers](network-rules.html#label-network-rule-identifiers) that should be allowed or
blocked.
...
[Azure Private Link](https://learn.microsoft.com/en-us/azure/private-link/private-link-overview). For details, see [Blocking public access — Recommended](private-internal-stages-azure.html#label-private-internal-stage-azure-block-public).
...
:   1. Sign in to [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in).
    ...
    7. In the Type drop-down, select the [type of identifier](network-rules.html#label-network-rule-identifiers) being defined in the network
...
You can create a network policy using [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in) or SQL:
...
SQL:
:   Execute the [ALTER ACCOUNT](../sql-reference/sql/alter-account) statement to set the [NETWORK\_POLICY](../sql-reference/parameters.html#label-network-policy)
...
[NETWORK\_POLICY](../sql-reference/parameters.html#label-network-policy) parameter for the user. For example, execute:
...
For details, refer to [Replication of security integrations & network policies across multiple accounts](account-replication-security-integrations).
