...
This section introduces some basic cell operations. When you [create a notebook](notebooks-create.html#label-notebooks-create), three example cells are
...
> ![Add new cell buttons at the bottom of the notebook.](../../_images/snowsight-ui-add-cell-bottom.png)
...
* Select the language dropdown menu and then select a different language.
  ...
  > ![Use the cell language drop down to change the cell language.](../../_images/snowsight-ui-change-cell.png)
...
1. (Option 1) Hover your mouse over the existing cell you want to move. Select the [![Notebooks drag and drop icon](../../_images/notebooks-drag-drop.png)](../../_images/notebooks-drag-drop.png) (drag and drop) icon on the left side of the cell
...
1. From the cell to duplicate, select the vertical ellipsis [![More actions for worksheet](../../_images/snowsight-worksheet-vertical-ellipsis.png)](../../_images/snowsight-worksheet-vertical-ellipsis.png) (more actions) menu.
2. Select Duplicate.
...
> [![Cell run details window.](../../_images/snowsight-ui-cell-run-details.png)](../../_images/snowsight-ui-cell-run-details.png)
...
In addition, you can use the same keyboard shortcuts that you use for worksheets. See [Perform tasks with keyboard shortcuts](../ui-snowsight-worksheets.html#label-snowsight-keyboard-shortcuts).
...
[![Screenshot of a Markdown cell showing Markdown text with an H1 header indicated with a # and a header of An example Markdown cell followed by body text of This is an example Markdown cell in a Snowflake Notebook. Below the raw Markdown content, the rendered Markdown appears with a different font.](../../_images/notebooks-markdown-editing.png)](../../_images/notebooks-markdown-editing.png)

To view only the formatted text, select the Done editing checkmark icon.

[![Screenshot of a Markdown cell that is showing only the rendered Markdown: a header of An example Markdown cell and body text of This is an example Markdown cell in a Snowflake Notebook.](../../_images/notebooks-markdown-display.png)](../../_images/notebooks-markdown-display.png)
...
| --- | --- | --- |
| Top level | CopyExpand  ``` # Top-level Header ```  Show lessSee more  Scroll to top | Header one in Markdown |
| 2nd-level | CopyExpand  ``` ## 2nd-level Header ```  Show lessSee more  Scroll to top | Header one in Markdown |
| 3rd-level | CopyExpand  ``` ### 3rd-level Header ```  Show lessSee more  Scroll to top | Header one in Markdown |
...
| --- | --- | --- |
| Italics | CopyExpand  ``` *italicized text* ```  Show lessSee more  Scroll to top | Italicized text in Markdown |
| Bold | CopyExpand  ``` **bolded text** ```  Show lessSee more  Scroll to top | Bolded text in Markdown |
| Link | CopyExpand  ``` [Link text](url) ```  Show lessSee more  Scroll to top | Link in Markdown |
...
| --- | --- | --- |
| Ordered list | CopyExpand  ``` 1. first item 2. second item   1. Nested first   2. Nested second ```  Show lessSee more  Scroll to top | Ordered list in Markdown |
| Unordered list | CopyExpand  ``` - first item - second item   - Nested first   - Nested second ```  Show lessSee more  Scroll to top | Unordered list in Markdown |
...
| --- | --- | --- |
| Python | CopyExpand  ``` ```python import pandas as pd df = pd.DataFrame([1,2,3]) ``` ```  Show lessSee more  Scroll to top | Python code snippet in Markdown |
| SQL | CopyExpand  ``` ```sql SELECT * FROM MYTABLE ``` ```  Show lessSee more  Scroll to top | SQL code snippet in Markdown |
...
| --- | --- | --- |
| Image | CopyExpand  ``` ![<alt_text>](<path_to_image>) ```  Show lessSee more  Scroll to top | Embedded image in Markdown |
...
| --- | --- | --- | --- |
| SQL | Python | `cell1` | Convert a SQL results table to a Snowpark DataFrame.  If you have the following in a SQL cell called `cell1`:  CopyExpand  ``` SELECT 'FRIDAY' as SNOWDAY, 0.2 as CHANCE_OF_SNOW UNION ALL SELECT 'SATURDAY',0.5 UNION ALL SELECT 'SUNDAY', 0.9; ```  Show lessSee more  Scroll to top  You can reference the cell to access the SQL result:  CopyExpand  ``` snowpark_df = cell1.to_df() ```  Show lessSee more  Scroll to top  Convert the result to a pandas DataFrame:  CopyExpand  ``` my_df = cell1.to_pandas() ```  Show lessSee more  Scroll to top |
...
| Reference cell type | Current cell type | Reference syntax | Example |
| --- | --- | --- | --- |
| SQL | SQL | `{{cell2}}` | For example, in a SQL cell named `cell1`, reference the cell results from `cell2`:  CopyExpand  ``` SELECT * FROM {{cell2}} where PRICE > 500 ```  Show lessSee more  Scroll to top |
| Python | SQL | `{{variable}}` | For example, in a Python cell named `cell1`:  **Using Python variable as a value**  CopyExpand  ``` c = "USA" ```  Show lessSee more  Scroll to top  You can reference the value of the variable `c` in a SQL cell named `cell2` by enclosing it in single quotes to ensure that it is treated as a value:  CopyExpand  ``` SELECT * FROM my_table WHERE COUNTRY = '{{c}}' ```  Show lessSee more  Scroll to top  **Using Python variable as an identifier**  If the Python variable represents a SQL identifier like a column or table name:  CopyExpand  ``` column_name = "COUNTRY" ```  Show lessSee more  Scroll to top  If the Python variable represents a SQL identifier, such as a column or table name (`column_name = "COUNTRY"`), you can reference the variable directly without quotes:  CopyExpand  ``` SELECT * FROM my_table WHERE {{column_name}} = 'USA' ```  Show lessSee more  Scroll to top  Make sure to differentiate between variables used as values (with quotes) and as identifiers (without quotes).  Note: Referencing Python DataFrames is not supported. |
...
* When referencing objects in SQL cells, you must use fully qualified object names, unless you are referencing object names in a specified
  database or schema. See [Changing the session context for a notebook](notebooks-sessions.html#label-notebooks-callers-rights).
* Notebook drafts are saved every three seconds.
* You can use [Git integration](notebooks-snowgit) to maintain notebook versions.
* You can configure an idle timeout setting to automatically shut down the notebook session once the setting is met. For information,
  see [Idle time and reconnection](notebooks-setup.html#label-notebooks-idle-time-property).
