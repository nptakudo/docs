...
notebook unexpectedly disconnects before the timeout, your ACCOUNTADMIN or the warehouse owner might have set the [STATEMENT\_TIMEOUT\_IN\_SECONDS](../../sql-reference/parameters.html#label-statement-timeout-in-seconds)
...
To allow the notebook to run longer, you can use the default warehouse [SYSTEM$STREAMLIT\_NOTEBOOK$WAREHOUSE](../warehouses-overview.html#label-default-warehouse) or
change the [STATEMENT\_TIMEOUT\_IN\_SECONDS](../../sql-reference/parameters.html#label-statement-timeout-in-seconds) parameter to a longer duration.

For details on the idle time setting, see [Idle time and reconnection](notebooks-setup.html#label-notebooks-idle-time-property).
...
Import the necessary package by following the instructions on the [Import Python packages to use in notebooks](notebooks-import-packages) page.
...
the Snowpark pandas API and a [Snowpark-optimized warehouse](../warehouses-snowpark-optimized). The
...
For more details, see [pandas on Snowflake in notebooks](notebooks-use-with-snowflake.html#label-notebooks-snowpark-pandas).
