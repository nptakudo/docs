...
To improve the performance of queries that use the function, you can optionally [enable FULL\_TEXT search optimization](search-optimization/enabling.html#label-enable-full-text-search) on a specific column or set of columns in a table. When you enable search
optimization, a new [search access path](search-optimization-service.html#label-search-optimization-service-how-it-works) is built and maintained.
