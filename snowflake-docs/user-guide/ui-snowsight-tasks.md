...
* Manage privileges on the task. For information, see [Manage object privileges with Snowsight](security-access-control-configure.html#label-snowsight-manage-object-privileges).
...
information about suspending and resuming tasks, see [Versioning of task runs](tasks-intro.html#label-versioning-of-task-runs).

## View and manage task graphs

Review a task graph to see a root task, its dependent tasks, and finalizer task in the format of a task graph. For more information about task graphs, see [Create a sequence of tasks with a task graph](tasks-graphs.html#label-task-dag). When you review a task graph, you can perform the following steps in Snowsight:
...
[Versioning of task runs](tasks-intro.html#label-versioning-of-task-runs).

[![Example task graph shown in Snowsight, with annotations showing the navigation to view the task graph as described in the following procedure.](../_images/ui-snowsight-task-graph.png)](../_images/ui-snowsight-task-graph.png)
...
* [ACCOUNT\_USAGE schema SNOWFLAKE database roles](../sql-reference/account-usage.html#label-account-usage-snowflake-db-roles)
...
[![Snowsight Tasks page with annotations for each section.](../_images/ui-snowsight-task-run-history.png)](../_images/ui-snowsight-task-run-history.png)
...
2. In the navigation menu, select Transformation » Tasks.
   ...
   [![On the Tasks page, view the list of task graphs.](../_images/ui-task-graph-runs.png)](../_images/ui-task-graph-runs.png)
...
> [![View the details of a task graph on the task graph detail page.](../_images/ui-task-graph-details.png)](../_images/ui-task-graph-details.png)
...
7. If the task is associated with a task graph, there is a Task Graph tab next to Task, which the following image demonstrates.
   ...
   [![On a specific task run history page, you can see the live status of the task run. If the task is associated with a task graph, you can also view the details of a task graph.](../_images/ui-task-run-history.png)](../_images/ui-task-run-history.png)
...
9. Select Retry to manually retry the failed task graph run.
   ...
   [![On a specific task graph run details page, you can select Retry to manually retry all failed task graph runs.](../_images/ui-retry-failed-tasks.png)](../_images/ui-retry-failed-tasks.png)
10. Select the refresh button to refresh the page. The failed attempts show up in the account-level task graph run details. Account-level task run history and task graph run history show the status of the most recent attempt with a 45-minute latency.
    ...
    [![After the page is refreshed, the failed attempts show up in the account-level task graph run details.](../_images/ui-retry-failed-tasks-attempts.png)](../_images/ui-retry-failed-tasks-attempts.png)
...
> [![Any previous failed or canceled attempts are shown next to the run status.](../_images/ui-retry-failed-tasks-task-run.png)](../_images/ui-retry-failed-tasks-task-run.png)
