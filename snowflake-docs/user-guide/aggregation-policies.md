...
url: "https://docs.snowflake.com/en/user-guide/aggregation-policies"
---
...
For more information about combining aggregation policies with an entity key, see [Implementing entity-level privacy with aggregation policies](aggregation-policies-entity-privacy).
...
* The query must aggregate the data. If the query uses an aggregation function, it must be one of the
  [allowed aggregation functions](#label-agg-policy-allowed-functions).
...
* If a column of an aggregation-constrained table is protected by a [projection policy](projection-policies), a query
  against that table cannot use the column as an argument of the COUNT function.
* [Recursive CTEs](queries-cte.html#label-recursive-common-table-expression) are not allowed in queries against an aggregation-constrained table or
  view.
...
* A query against an aggregation-constrained table cannot use a [correlated subquery](querying-subqueries.html#label-correlated-vs-uncorrelated-subqueries)
...
The body of an aggregation policy uses two functions to define the constraints of the policy: [NO\_AGGREGATION\_CONSTRAINT](#label-aggregation-policy-no-aggregation-constraint) and [AGGREGATION\_CONSTRAINT](#label-aggregation-policy-aggregation-constraint). When the conditions of the body call one of these functions, the return value from the function determines how
...
:   Use the body’s expression to call the AGGREGATION\_CONSTRAINT function to require that queries aggregate data in order to return results.
    Use the MIN\_GROUP\_SIZE argument to specify how many rows or [entities](aggregation-policies-entity-privacy) must be
...
:   Policy administrators can define the SQL expression of an aggregation policy so different queries have different restrictions based on
    ...
    * The [POLICY\_CONTEXT](../sql-reference/functions/policy_context) function helps you evaluate whether a policy body is returning the correct value
      when a context function returns a certain value. The POLICY\_CONTEXT function simulates query results based upon a specified value of
...
:   The following aggregation functions are allowed in a query against an aggregation-constrained table:
    ...
    * [SUM](../sql-reference/functions/sum)
    ...
    |  |  |  |
    | --- | --- | --- |
    | * [CEIL](../sql-reference/functions/ceil) * [CONCAT , ||](../sql-reference/functions/concat) * [DATE\_TRUNC](../sql-reference/functions/date_trunc) * [DECRYPT](../sql-reference/functions/decrypt) * [ENCRYPT](../sql-reference/functions/encrypt) | * [FLOOR](../sql-reference/functions/floor) * [GET](../sql-reference/functions/get) * [INITCAP](../sql-reference/functions/initcap) * [LOWER](../sql-reference/functions/lower) * [LTRIM](../sql-reference/functions/ltrim) | * [MD5 , MD5\_HEX](../sql-reference/functions/md5) * [ROUND](../sql-reference/functions/round) * [RTRIM](../sql-reference/functions/rtrim) * [TRIM](../sql-reference/functions/trim) * [UPPER](../sql-reference/functions/upper) |
...
UNION ALL
:   A query can use [UNION ALL](../sql-reference/operators-query.html#label-query-operators-union) to combine results of two subqueries, even if one or more of the queried
...
[row access policies](security-row-intro), and [projection policies](projection-policies).
...
[secure views](views-secure) even if they are not.
...
* A table or view contained in the primary database has a [dangling reference](database-replication-considerations.html#label-database-replication-dangling-references) to an
