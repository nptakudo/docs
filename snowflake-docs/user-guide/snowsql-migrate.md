...
url: "https://docs.snowflake.com/en/user-guide/snowsql-migrate"
---
...
Similarly to SnowSQL, Snowflake CLI provides [binary installers](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-package-managers). Additionally, it also lets you install the software using [homebrew](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-homebrew) and [pip](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-pip).

Currently, Snowflake CLI supports following platforms:

* macOS (arm)

  + [Package installer](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-macos-installer)
  + [Homebrew](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-homebrew)
  + [PyPi (pip)](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-advanced-install)
* macOS (x86\_64):
...
* Linux (x86\_64 & aarch64):
  ...
  + [deb package installer](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-linux-package-managers)
  + [rpm package installer](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-linux-package-managers)
  + [PyPi (pip)](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-advanced-install)
* Windows (64bit):

  + [Installer](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-install-windows-installer)
  + [PyPi (pip)](../developer-guide/snowflake-cli/installation/installation.html#label-snowcli-advanced-install)
...
* The [snow helpers import-snowsql-connections](../developer-guide/snowflake-cli/command-reference/helpers-commands/import-snowsql-connections) command uses an interactive menu to let you choose which SnowSQL connections you want to import.
  For more information, see [Import connections from SnowSQL](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-import-connections-snowsql).
...
* SnowSQL
  ...
  SnowSQL is configured by its [configuration file](snowsql-config.html#label-configuring-snowsql), which is a file in TOML format that contains connection configurations, various settings of the tool, and variables that can be used in SQL queries.
  ...
  For more information, see [Connection parameters reference](snowsql-start.html#label-snowsql-connection-parameters).
* Snowflake CLI
...
The location of the default Snowflake CLI configuration depends on your system and is determined by the order specified in [Location of the .toml configuration file](../developer-guide/snowflake-cli/connecting/configure-cli.html#label-cli-config-locations).
...
* [Import connections from SnowSQL](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-import-connections-snowsql) documentation

You can import all of your SnowSQL connections with the `snow helpers import-snowsql-connections` command.
For more information, see [Import connections from SnowSQL](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-import-connections-snowsql) and the [snow helpers import-snowsql-connections](../developer-guide/snowflake-cli/command-reference/helpers-commands/import-snowsql-connections) command reference.
...
* SnowSQL
  ...
  The default connection is configured in the SnowSQL [configuration file](snowsql-config.html#label-configuring-snowsql), and connection settings are defined directly in the [[connections]](snowsql-start.html#label-connecting-default-connection-settings) section.
* Snowflake CLI

  The default connection is configured in the Snowflake CLI [configuration file](../developer-guide/snowflake-cli/connecting/configure-cli) as a named connection with the name `default_connection_name`, set at the top level of configuration (see [Set the default connection](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-cli-set-default-connection)).
...
2. To add the connection to Snowflake CLI, use one of the following methods:
   ...
   * Manually edit the Snowflake CLI configuration file, as follows:
     ...
     3. Change the names of the following parameters, as shown:
        ...
        + `rolename` to `role`
     4. Add or set `default_connection_name = "your_connection_name"` setting at the top level of the configuration file (see [Set the default connection](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-cli-set-default-connection)).
   * Use the `snow connection add` and `snow connection set-default` commands.
     For more information, see [Manage or add your connections to Snowflake with the snow connection commands](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-snow-connection-command).
...
* SnowSQL
  ...
  Named connections are configured in the SnowSQL [configuration file](snowsql-start.html#label-connecting-named-connection). Each named connection has its own `[connections.your_connection_name]` section.
* Snowflake CLI

  Snowflake CLI uses almost the same format to [configure named connections](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-define-connections). You can copy them from the SnowSQL configuration and rename the parameters as specified in the default connection.
...
* [Configure logging](../developer-guide/snowflake-cli/connecting/configure-cli.html#label-snowcli-configure-logging) documentation

To manually configure logging for Snowflake CLI, see the [Configure logging](../developer-guide/snowflake-cli/connecting/configure-cli.html#label-snowcli-configure-logging) documentation.
...
* [Use environment variables for Snowflake credentials](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-environment-creds) documentation
* [Use variables in SQL](../developer-guide/snowflake-cli/project-definitions/use-sql-variables) documentation
...
If you’re using environment variables to connect to Snowflake, for more information, see [connecting to Snowflake with environment variables](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-environment-creds). Also, see information about possibilities for [configuring environment variables](../developer-guide/snowflake-cli/connecting/configure-cli.html#label-snowcli-env-var-support) in the Snowflake CLI documentation.
...
* To connect using the default connection, passing some parameters as environment variables:
  ...
  + Snowflake CLI
    ...
    Note that the names of environment variables might differ. For more information, see [Use environment variables for Snowflake credentials](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-environment-creds).
...
Snowflake CLI currently does not support all of the [SnowSQL output formatting options](snowsql-use.html#label-snowsql-export-data). Snowflake CLI does let you save query results as either a formatted table or as JSON. Although CSV and other formats are not yet available, you can use external tools, such as [jq](https://jqlang.org/), to covert data from JSON other formats.
...
| Ability to generate and display a JWT token based on the `user`, `account`, and `private-key-path` parameters. | Use the `snow connection generate-jwt` command. For more information, see [Use a private key file for authentication](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-private-key). |
...
| Query templating with the option to provide variables using the `--variable` command-line option, such as `--variable db_key=$DB_KEY`. | Snowflake CLI supports SQL variables in SQL templates and in snowflake.yml project definition files. For more information, see [Using variables for SQL templates](../developer-guide/snowflake-cli/sql/execute-sql.html#label-cli-sql-templating) and [Storing variables in the snowflake.yml project definition file](../developer-guide/snowflake-cli/sql/execute-sql.html#label-cli-sql-env-vars). |
| [Interactive SQL shell mode](snowsql-use.html#label-using-commands). | Use [interactive mode](../developer-guide/snowflake-cli/sql/execute-sql.html#label-snowcli-sql-interactive-mode). Support for asynchronous queries will be added at a later date. |
| Include, or source, one or more SQL files from another SQL file:  CopyExpand  ``` !source file1.sql; !source file2.sql; !source http://example.com/my.sql ```  Show lessSee more  Scroll to top | Snowflake CLI supports nesting SQL scripts with template support. For more information, see [Working with SQL query commands](../developer-guide/snowflake-cli/sql/execute-sql.html#label-snowcli-sql-query-commands). |
| Display EXIT\_ON\_ERROR error codes. | Use the `--enhanced-exit-codes` command-line option, or set the `SNOWFLAKE_ENHANCED_EXIT_CODES` environment variable to `1` to send the enhanced return codes for all `snow sql` commands. For more information, see [Enhanced error codes](../developer-guide/snowflake-cli/command-reference/sql-commands/sql.html#label-snowcli-enhanced-error-codes). |
