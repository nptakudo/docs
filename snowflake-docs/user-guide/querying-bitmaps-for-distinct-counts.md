...
[![Logical representation of a bitmap](../_images/querying-bitmaps.png)](../_images/querying-bitmaps.png)
...
| --- | --- |
| CopyExpand  ``` SELECT   COUNT(DISTINCT my_column) FROM my_table; ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT SUM(cnt) FROM (   SELECT     BITMAP_COUNT(BITMAP_CONSTRUCT_AGG(BITMAP_BIT_POSITION(my_column))) cnt   FROM my_table   GROUP BY BITMAP_BUCKET_NUMBER(my_table) ); ```  Show lessSee more  Scroll to top  Note that if the range of values in `my_column` is 0 to 32,768, you can use this simpler statement instead:  CopyExpand  ``` -- If the full value range of my_column fits into the bitmap: --   MIN(my_column) >= 0 AND MAX(my_column) < 32,768 SELECT   BITMAP_COUNT(BITMAP_CONSTRUCT_AGG(my_column)) FROM my_table; ```  Show lessSee more  Scroll to top |
...
| Example With COUNT(DISTINCT <expression>) | Example With Bitmap Functions |
| --- | --- |
| CopyExpand  ``` SELECT   my_key_1,   my_key_2,   COUNT(DISTINCT my_column) FROM my_table GROUP BY my_key_1, my_key_2; ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT my_key_1, my_key_2, SUM(cnt) FROM (   SELECT     my_key_1,     my_key_2,     BITMAP_COUNT(BITMAP_CONSTRUCT_AGG(BITMAP_BIT_POSITION(my_column))) cnt   FROM my_table   GROUP BY my_key_1, my_key_2, BITMAP_BUCKET_NUMBER(my_column) ) GROUP BY my_key_1, my_key_2; ```  Show lessSee more  Scroll to top |
