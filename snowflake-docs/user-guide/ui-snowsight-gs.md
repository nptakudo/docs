...
url: "https://docs.snowflake.com/en/user-guide/ui-snowsight-gs"
---
...
see [Warehouse considerations](warehouses-considerations). Features that execute queries against a warehouse incur compute costs. For
strategies to reduce usage credits, see [Optimizing cost](cost-optimize).
...
See [Getting started with worksheets](ui-snowsight-worksheets-gs).
...
2. Provide your [account identifier](admin-account-identifier) or account URL.
...
After [completing the configuration to use private connectivity](#label-ui-snowsight-config-private-connectivity),
...
1. In the browser location bar, enter either of the following URLs:
   ...
   * `cloud_region_id` is the identifier for the cloud region, which is controlled by the cloud platform.
   ...
   For details, see [Locate your Snowflake account information in Snowsight](#label-snowsight-account-details) and [Format 1 (preferred): Account name in your organization](admin-account-identifier.html#label-account-name).
...
[Enable notifications from Trust Center](ui-snowsight-profile.html#label-snowsight-enable-notifications-trust-center).
...
For more information see [Enroll in multi-factor authentication (MFA)](ui-snowsight-profile.html#label-snowsight-set-up-mfa).
...
[![Switch Account](../_images/snowsight-gs-account-selector.png)](../_images/snowsight-gs-account-selector.png)
...
To learn more about roles and privileges, see [Overview of Access Control](security-access-control-overview).
...
4. By default, changing the value of `regionless-snowsight-privatelink-url` only updates the connection URL used internally by
   ...
   * A reference to [Private connectivity URLs](organizations-connect.html#label-snowsight-private-connectivity-urls).
