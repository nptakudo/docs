...
appears in the [data lineage graph](ui-snowsight-lineage).
...
If you provide a [symmetric string secret](../sql-reference/sql/create-secret.html#label-symmetric-secret) to `consistency_secret` in GENERATE\_SYNTHETIC\_DATA, join key
...
If you use provide a SECRET object to GENERATE\_SYNTHETIC\_DATA, you need the READ or OWNERSHIP privilege on that SECRET.
...
You must [accept the Anaconda terms and conditions](../developer-guide/udf/python/udf-python-packages.html#label-accept-anaconda-terms) in your Snowflake account in order to enable this
feature.

## Recommendations

* Use a medium [Snowpark-optimized warehouse](warehouses-snowpark-optimized).
...
This example uses the [Snowflake Sample Data database SNOWFLAKE\_SAMPLE\_DATA](sample-data-using). If you don’t see it in
