...
| Stream | The role that queries a given [stream](streams-intro.html#label-stream-required-privileges). |
...
2. Determine if the specified role is in invoker role hierarchy. For example, if the policy is set on a view, the specified role must
   be in the view owner role hierarchy to return TRUE. For details, see the [usage notes](../sql-reference/functions/is_granted_to_invoker_role.html#label-is-granted-to-invoker-role-notes).
3. Run a test query to verify.
...
(e.g. view owner, [INVOKER\_ROLE](../sql-reference/functions/invoker_role)). Uses cases of this type are typically more complicated than simply
...
Before implementing any of these functions in a [masking policy](security-column-intro), it is important to consider
...
[External Tokenization](security-column-ext-token-intro). Tokenization does not result in collisions because there is
