...
If the task has a predecessor task (in a [task graph](tasks-graphs.html#label-task-dag)), verify whether the predecessor task completed successfully.
...
If the task is a child task in a task graph, check that the predecessor tasks (parent tasks) ran to completion successfully. If A parent task failed to run to completion, any child tasks are skipped. For more information, see [Create a sequence of tasks with a task graph](tasks-graphs.html#label-task-dag).
