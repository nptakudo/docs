...
url: "https://docs.snowflake.com/en/user-guide/oauth-local-applications"
---
...
[Snowflake OAuth](oauth-snowflake-overview) is implemented by creating a *security integration* that defines an interface
...
* It is a straightforward replacement for applications that are currently using passwords only to authenticate users. Snowflake is
  [deprecating single-factor passwords](security-mfa-rollout), so Snowflake OAuth for local applications provides a path to
...
For more information about making REST requests to Snowflake’s authorization and token endpoints, see [Call the OAuth endpoints](oauth-custom.html#label-snowflake-oauth-endpoints).
...
* Requests to the authorization and token endpoints must implement Proof Key for Code Exchange (PKCE). For more information, see
  [Proof key for code exchange](oauth-custom.html#label-proof-key-for-code-exchange).
