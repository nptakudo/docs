...
url: "https://docs.snowflake.com/en/user-guide/functions-window-using"
---
...
To see how this works, first [create and load the menu\_items table](../sql-reference/functions/stddev.html#label-create-and-load-menu-items-table), which contains the cost of goods
...
[Analyzing time-series data](querying-time-series-data).
...
a window frame. For details, see [Usage notes for window frames](../sql-reference/functions-window-syntax.html#label-window-frames).
...
* For explicit frame syntax, see the `windowFrameClause` under [Syntax](../sql-reference/functions-window-syntax.html#label-window-syntax-and-usage). You can define open-ended boundaries: from the beginning of the partition to the current row; from the current row to the end of the partition; or completely “unbounded” end to end. Alternatively, you can use explicit offsets (inclusive) that are relative to the current row in the partition.
* Implicit frames are used by default when the OVER clause does not include a `windowFrameClause`. The default frame depends on the function in question. See also [Usage notes for window frames](../sql-reference/functions-window-syntax.html#label-window-frames).
...
See also [RANGE BETWEEN example with explicit numeric offsets](../sql-reference/functions-window-syntax.html#label-range-between-examples).
...
This example selects from a small table named `menu_items`. See [Create and load the menu\_items table](../sql-reference/functions/stddev.html#label-create-and-load-menu-items-table).
