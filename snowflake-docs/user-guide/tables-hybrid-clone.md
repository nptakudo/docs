...
url: "https://docs.snowflake.com/en/user-guide/tables-hybrid-clone"
---
...
* To run point-in-time restore operations. Cloning works in combination with [Time Travel](data-time-travel),
  which by default creates implicit continuous backups. After [setting a data retention period](data-time-travel.html#label-data-retention-specifying),
...
If you try to create a schema by cloning another schema, and the source schema has one or more hybrid tables, the command fails. However, you can clone the schema by using the [IGNORE HYBRID TABLES](../sql-reference/sql/create-clone.html#label-ignore-hybrid-tables-parameter) parameter to explicitly skip the hybrid tables in the schema. This parameter also works for creating database clones. For example:
...
* Cloning hybrid tables is a size-of-data operation, while cloning standard tables is a metadata-only operation. This difference has an impact on compute cost, storage cost, and performance.
  ...
  + Cloning performance is similar to that of optimized direct bulk loading with CREATE TABLE AS SELECT. See [Loading data](tables-hybrid-create.html#label-create-loading-data).
...
[![Query History filtered by SQL Text to show a series of four DELETE operations.](../_images/query_history_filter_deletes.png)](../_images/query_history_filter_deletes.png)
