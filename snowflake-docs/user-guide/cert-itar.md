...
For details, refer to [SnowGov Regions](intro-regions.html#label-us-gov-regions).
...
Snowflake supports customer ITAR compliance in the Snowflake [SnowGov regions](intro-regions.html#label-us-gov-regions) by limiting region access to
...
For more information about the service offerings that are currently authorized, see [U.S. regions supporting public sector workloads](intro-regions.html#label-us-gov-regions).

## Export-controlled Data and Cross-region Features

Unlike commercial regions, where multiple regions belong to the same [region group](admin-account-identifier.html#label-region-groups), each government region
...
As an example, [replication](account-replication-intro) is only possible to the boundary of a region group. Because a database cannot be replicated to a region outside
...
If your Snowflake account is in a [U.S. government region](intro-regions.html#label-us-gov-regions) and you want to access data products that are
