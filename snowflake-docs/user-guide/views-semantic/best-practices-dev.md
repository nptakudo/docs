...
[semantic views](overview). These recommendations are primarily intended for data engineering and data science
...
are not required. This behavior is consistent with the privileges [required to query standard views](../views-introduction.html#label-views-privileges).
...
[Dynamic Data Masking policies](../security-column-ddm-intro) and [row access policies](../security-row-intro)
...
* Create a semantic view in Snowsight:
  ...
  + The wizard approach is recommended for initial setup, and includes automatic creation of synonyms, sample values, and column descriptions.
    For instructions, see [Using Snowsight to create and manage semantic views](ui).
...
and DESCRIBE commands, and users can access them downstream via [SQL SELECT statements](querying)
and in the following ways:

* Directly through the [Cortex Analyst](../snowflake-cortex/cortex-analyst) user interface
* Through [Streamlit](../../developer-guide/streamlit/about-streamlit) or other custom applications that use
  the Cortex Analyst API and/or [generate SELECT FROM SEMANTIC\_VIEW statements](querying)
* Through Cortex Agents via Cortex Analyst (semantic views must be added to a new or existing
  [agent](../snowflake-cortex/cortex-agents-manage.html#label-snowflake-agents-modify-agents))
...
You can use the [SYSTEM$CREATE\_SEMANTIC\_VIEW\_FROM\_YAML](../../sql-reference/stored-procedures/system_create_semantic_view_from_yaml) stored procedure for conversion. If you need bulk conversion or integration
...
To export a native semantic view back to YAML, you can use the [SYSTEM$READ\_YAML\_FROM\_SEMANTIC\_VIEW](../../sql-reference/functions/system_read_yaml_from_semantic_view) function.
...
[data sharing](../../guides-overview-sharing). You can create [secure views](../views-secure) based on semantic views, and sharing
...
see [Integration with dbt projects](#label-semantic-views-dbt-integration). Support for a similar process using the [Snowflake Terraform provider](../terraform) is planned.
...
This package works natively with [dbt Projects on Snowflake](../data-engineering/dbt-projects-on-snowflake) or any dbt installation that has
