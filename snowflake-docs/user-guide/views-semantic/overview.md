...
You can use Semantic Views in [Cortex Analyst](../snowflake-cortex/cortex-analyst) and
[query these views](querying) in a SELECT statement. You can also [share Semantic Views](sharing-semantic-views) in [private listings](../../collaboration/provider-listings-creating-publishing.html#label-listings-create), in public listings on the [Snowflake Marketplace](https://app.snowflake.com/_deeplink/marketplace), and in [organizational listings](../collaboration/listings/organizational/org-listing-about).

![Diagram illustrating how Semantic Views provide a more meaningful interface to tools and data](../../_images/semantic-views-diagram.png)

To [create and manage Semantic Views](#label-semantic-views-interfaces), you can use SQL commands (such as
[CREATE SEMANTIC VIEW](../../sql-reference/sql/create-semantic-view)) or a wizard in Snowsight that guides you through the process of creating
a semantic view.
...
* **For AI applications:** Semantic views improve accuracy by combining LLM reasoning with rule-based definitions. Currently,
  [Cortex Analyst](../snowflake-cortex/cortex-analyst) reads the information captured in the semantic view
...
For more information about these concepts, see the [YAML Specification for Semantic Views](semantic-view-yaml-spec).

## Interfaces for working with Semantic Views

You can use the following interfaces to create, manage, and use Semantic Views:

* **SQL commands**: You can use SQL commands to create and manage Semantic Views directly. For information, see [Using SQL commands to create and manage semantic views](sql).

  You can also execute a SELECT statement to [query a semantic view](querying).
* **Snowsight**: You can use a wizard in Snowsight that guides you through the process of creating a semantic view.
  You can also upload a [YAML specification](semantic-view-yaml-spec) that defines your semantic view. For information,
  see [Using Snowsight to create and manage semantic views](ui).
* **Cortex Analyst REST API**: To use a semantic view with Cortex Analyst, you specify the view in the REST API request. For
  information, see [Cortex Analyst REST API](../snowflake-cortex/cortex-analyst/rest-api).

## Limitations

* Semantic views are not supported for [replication](../account-replication-intro).
...
4. Use the semantic view in the following ways:
   ...
   * Use Cortex Analyst for natural language queries of your semantic view.
     ...
     You can use the [Cortex Analyst REST API](../snowflake-cortex/cortex-analyst/rest-api) to perform a natural
     language query that uses your semantic view.

     If you need to monitor the REST API requests that use your semantic view, see
     [Cortex Analyst administrator monitoring](../snowflake-cortex/cortex-analyst/admin-observability).
   * Query the semantic view in a SELECT statement. For information, see [Querying semantic views](querying).

## Additional information about Semantic Views

For additional information about Semantic Views, see the following topics:

* [Using Snowsight to create and manage semantic views](ui)
* [Using SQL commands to create and manage semantic views](sql)
* [How Snowflake validates semantic views](validation-rules)
* [Example of using SQL to create a semantic view](example)
* [Querying semantic views](querying)

For information about the privileges required to work with Semantic Views, see the following sections:

* [Privileges required to create or replace a semantic view](sql.html#label-semantic-views-create-privileges)
* [Privileges required to query a semantic view](querying.html#label-semantic-views-privileges-select)
* [Granting privileges on semantic views](sql.html#label-semantic-views-privileges)
