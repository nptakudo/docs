---
title: "Viewing accounts in your organization"
url: "https://docs.snowflake.com/en/user-guide/organizations-manage-accounts-view"
---

# Viewing accounts in your organization

[As the organization administrator](organization-administrators), you can view a list of all accounts created in your
organization through the web interface or using SQL:

> [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in):
> :   In the navigation menu, select Admin » Accounts.
>     You can search and filter active and dropped accounts in your organization.
>
> SQL:
> :   Execute a [SHOW ACCOUNTS](../sql-reference/sql/show-accounts) command.
