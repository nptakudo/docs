...
In the [previous step](data-load-s3-config), if you followed the instructions to configure an AWS IAM role with the required policies and permissions
to access your external S3 bucket, you have already created an S3 stage. You can skip this step and continue to [Copying data from an S3 stage](data-load-s3-copy).
...
that was created in [Option 1: Configure a Snowflake storage integration to access Amazon S3](data-load-s3-config-storage-integration) to access the S3 bucket. The stage references a named file
