...
or unmanaged schema. For details, see [CREATE SCHEMA privileges](../sql-reference/sql/create-schema.html#label-create-schema-ac).
...
In a table, a column can reference a [sequence](querying-sequences) that generates default values. When a table is cloned,
...
A table can have a subset of columns designated as a [clustering key](tables-clustering-keys) to co-locate similar rows in the
...
[Automatic Clustering](tables-auto-reclustering) is suspended for the new table. To resume automatic clustering for the new table, run the
following command:
...
* Internal named stages are cloned if you use the INCLUDE INTERNAL STAGES clause. For more information,
  see the [internal stage cloning usage notes](../sql-reference/sql/create-clone.html#label-create-clone-internal-stages-notes).
...
For information about how storage works for cloned objects, see [Cloned table, schema, and database storage](tables-storage-considerations.html#label-cloning-tables).

For information about Iceberg table storage, see [Storage for Apache Iceberg™ tables](tables-iceberg-storage).
...
[Use DML commands](tables-iceberg-manage.html#label-tables-iceberg-manage-dml).
...
You can clone tables that have the [Search optimization service](search-optimization-service) enabled. When you do, the corresponding search
access path is a [zero-copy clone](tables-storage-considerations.html#label-cloning-tables). However, if the cloned search access path isn’t up-to-date,
...
[Cloning the table, schema, or database](search-optimization/working-with-tables.html#label-search-optimization-and-cloning).
...
clone) are inaccessible. This behavior is consistent with [Time Travel](data-time-travel) for tables. If a table is
...
[suspended](alerts.html#label-alerts-suspend-resume) by default.
...
> Also see:
>
> * Cloning external tables and [masking policies](security-column-intro.html#label-security-column-intro-ext-table).
> * Cloning external tables and [row access policies](security-row-intro.html#label-security-row-intro-ext-table).
...
Cloning a table or view that is protected by [differential privacy](diff-privacy/differential-privacy-overview) results
...
conditions. For more information, see [Limitations on cloning](../developer-guide/udf/java/udf-java-limitations.html#label-limitations-on-cloning-java-udfs).
...
is cloned. Cloning of instances of other Snowflake [classes](../sql-reference-classes.html#label-available-classes) is *not* supported.

### Cloning and WORM backups

The backup set and backup policy objects that are used in [Write Once Read Many (WORM) backups](backups)
...
The [data retention period](data-time-travel.html#label-data-retention-specifying) specifies the number of days for which Snowflake retains historical
data for performing
...
This section provides information to consider when using [Time Travel](data-time-travel) to clone objects at a specific time/point in the
past.
...
>   CREATE <object> … CLONE command. For more information, see [Child objects and data retention time](#label-child-objects-and-data-retention-time).
...
> * If the [IGNORE HYBRID TABLES parameter](../sql-reference/sql/create-clone.html#label-ignore-hybrid-tables-parameter) isn’t specified and any hybrid tables exist in the
>   specified database or schema.

#### Child objects and data retention time

If a child object (for example, a table) has a shorter [data retention period](data-time-travel.html#label-time-travel-data-retention-period) than
...
For example, the [data retention period](data-time-travel.html#label-data-retention-specifying) for database `db1` is seven days and the
...
As a workaround, use the [IGNORE TABLES WITH INSUFFICIENT DATA RETENTION](../sql-reference/sql/create-clone.html#label-ignore-tables-with-insufficient-data-retention)
...
is executed or at a specified time/point in the past using [Time Travel](data-time-travel). An object clone inherits any other metadata,
...
For more information, see [Replication and cloning](account-replication-considerations.html#label-replication-and-cloning).
