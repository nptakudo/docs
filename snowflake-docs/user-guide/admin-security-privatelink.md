...
[private endpoints](../sql-reference/external-functions-creating-aws-planning.html#label-external-functions-aws-endpoint-type).
...
[Pinning private connectivity endpoints for inbound traffic](pin-private-endpoints).
...
* If you use an earlier client version, then the OCSP cache server host name takes the form `ocsp.region_id.privatelink.snowflakecomputing.com`
  without an [account identifier](admin-account-identifier).
...
* Configure an AWS VPC interface endpoint for [internal stages](private-internal-stages-aws). This option is recommended.
...
For more information, see [SnowCD](snowcd) and [SYSTEM$ALLOWLIST\_PRIVATELINK](../sql-reference/functions/system_allowlist_privatelink).

Otherwise, connect to Snowflake with your private connectivity account [URL](organizations-connect.html#label-connecting-via-url).
...
For more information, see [Controlling network traffic with network policies](network-policies).
...
* For all the other clients, *don’t* specify the entire host name.
  Instead, specify the [account identifier](admin-account-identifier) with the `privatelink` segment, which is `<account_identifier>.privatelink`. Snowflake concatenates this name with `snowflakecomputing.com` to dynamically construct the host name.
