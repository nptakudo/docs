...
For details, see [Introduction to replication and failover across multiple accounts](account-replication-intro).

These objects and services are supported across [regions](intro-regions) and across
...
[connection URL](client-redirect) in the SAML2 security integration definition.
...
1. If the source account is already configured for [Database Failover/Failback and Client Redirect](replication-intro),
...
5. Verify that SSO to the source account works for users that are specified in the Snowflake application in Okta and users in the source
   ...
   [Supported SSO workflows](admin-security-fed-auth-overview.html#label-fed-auth-sso-workflow).
6. In the source account, if a failover group does not already exist, [create](../sql-reference/sql/create-failover-group.html#label-create-failover-group) a failover group to
   ...
   If a failover group already exists, [alter](../sql-reference/sql/alter-failover-group.html#label-alter-failover-group) the failover group to include integrations.
...
2. If an Okta SCIM security integration is already configured in the source account, skip to the next step.
   ...
   Otherwise, configure an [Okta SCIM](scim-okta) security integration in the source account.
   ...
   Be sure to update the Okta SCIM application for Snowflake. For details, see [Okta configuration](scim-okta.html#label-scim-okta-configuration).
...
4. If the failover group already specifies `security integrations`, skip to the next step. This would be true if you have already
   ...
   [SAML SSO in the target account](#label-account-replication-security-integration-saml2) (in this topic).
...
5. At this point, you can optionally refresh the secondary failover group as shown in the
   [target account steps for SCIM](#label-target-account-steps-scim) to ensure the new user in the source account is in the target
   account.
...
6. Update the remote service associated with the API integration in the cloud platform that hosts your Snowflake account.
   ...
   For details, refer to [Updating the remote service for API integrations](account-replication-config.html#label-update-remote-service-for-api-integrations).
