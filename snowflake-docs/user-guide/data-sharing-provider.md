...
* You can share data across regions and cloud platforms. For more information,
  see [Share data securely across regions and cloud platforms](secure-data-sharing-across-regions-platforms).
* A share can include data from multiple databases. For more information, see [Share data from multiple databases](data-sharing-multiple-db).
...
* For data security and privacy reasons, only [secure views](views-secure) are supported in shares at this time.
...
* [Storage lifecycle policies](storage-management/storage-lifecycle-policies) aren’t supported on shared tables. If you need to manage data retention
...
For more details about the CREATE SHARE privilege, see [Enable non-ACCOUNTADMIN roles to perform data sharing tasks](security-access-privileges-shares).
...
use [secure views](views-secure), [secure materialized views](views-materialized)
...
Secure objects are defined similar to standard objects, using either the corresponding [CREATE <object>](../sql-reference/sql/create)
...
* When defining a secure object to share with consumer accounts, a key/vital additional step to perform
  ...
  this validation, Snowflake provides the [SIMULATED\_DATA\_SHARING\_CONSUMER](../sql-reference/parameters.html#label-simulated-data-sharing-consumer) session parameter.
...
For a detailed example, see [Use secure objects to control data access](data-sharing-secure-views).
...
:   When a stream on a local table is not consumed regularly, Snowflake temporarily extends the data retention period for the source table
    ...
    for any shared table, or any underlying table for a shared view, set the [DATA\_RETENTION\_TIME\_IN\_DAYS](../sql-reference/parameters.html#label-data-retention-time-in-days) parameter for
    the table.
...
[Tag inheritance](object-tagging/inheritance.html#label-object-tagging-lineage) applies to tagged objects in the shared database. For example, if a provider sets a tag
...
> * Publish a listing in a [data exchange](data-exchange-managing-data-listings).
...
you must create your share using SQL. For more information, see [Share data from multiple databases](data-sharing-multiple-db).
...
[How to share database objects](data-sharing-gs.html#label-data-sharing-share-options).
...
If you plan to securely share data with data consumers across different [regions](intro-regions) or
[cloud platforms](intro-cloud-platforms), note that replicating a primary database is blocked if the database
contains some types of objects. For a full list of objects that cause refresh operations to fail, see
[Current limitations of replication](account-replication-intro.html#label-replication-limitations).
...
create your share using SQL. See [Share data from multiple databases](data-sharing-multiple-db).
