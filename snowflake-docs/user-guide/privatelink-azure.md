...
[![Architecture overview](../_images/azure-privatelink-overview.png)](../_images/azure-privatelink-overview.png)
...
6. After the Private Endpoint displays a CONNECTION STATE value of Approved, test your connection to Snowflake with
   [SnowCD (Connectivity Diagnostic Tool)](snowcd) and [SYSTEM$ALLOWLIST\_PRIVATELINK](../sql-reference/functions/system_allowlist_privatelink).
7. Connect to Snowflake using your private connectivity account URL.

To further harden your security posture, Snowflake recommends [Pinning private connectivity endpoints for inbound traffic](pin-private-endpoints) for your Snowflake account.
...
2. Navigate to the Azure portal. Search for Private Link and click Private Link.
   ...
   [![Private Link Preview](../_images/azure-privatelink-preview.png)](../_images/azure-privatelink-preview.png)
3. Click Private endpoints and then click Add.

   [![Select Private link endpoint](../_images/azure-privatelink-endpoint.png)](../_images/azure-privatelink-endpoint.png)
...
> [![Endpoint Basics](../_images/azure-privatelink-endpoint-basics.png)](../_images/azure-privatelink-endpoint-basics.png)
...
1. In the Resource section, complete the Connection method and the Resource ID or alias Field fields.
   ...
   * For Connection Method, select the Connect to an Azure resource by resource ID or alias.
     ...
     [![Endpoint Resource](../_images/azure-privatelink-endpoint-resource.png)](../_images/azure-privatelink-endpoint-resource.png)
...
2. Return to the Private endpoints section and allow a few minutes to wait. On approval, the Private Endpoint displays a
   ...
   [![Endpoint approved state](../_images/azure-privatelink-endpoint-approved.png)](../_images/azure-privatelink-endpoint-approved.png)
...
4. DNS Setup. All requests to Snowflake need to be routed via the Private Endpoint. Update your DNS to resolve the Snowflake account and
   ...
   * To get the endpoint IP address, navigate to Azure portal search bar and enter the name of the endpoint
     ...
     [![DNS endpoint setup](../_images/azure-privatelink-dns-endpoint.png)](../_images/azure-privatelink-dns-endpoint.png)
   * Copy the value for the Private IP address (i.e. `10.0.27.5`).

     [![DNS private IP address](../_images/azure-privatelink-dns-private-ip.png)](../_images/azure-privatelink-dns-private-ip.png)
   * Configure your DNS to have the appropriate endpoint values from the [SYSTEM$GET\_PRIVATELINK\_CONFIG](../sql-reference/functions/system_get_privatelink_config)
     ...
     connectivity. For a description of the possible values, see [Return values](../sql-reference/functions/system_get_privatelink_config.html#label-get-privatelink-config-output).
     ...
     URL redirects. For information, see [Snowsight & Private Connectivity](ui-snowsight-gs.html#label-ui-snowsight-gs-private-connectivity).
...
5. After verifying your outbound firewall settings and DNS records include your Azure Private Link account and OCSP URLs, test your
   connection to Snowflake with [SnowCD (Connectivity Diagnostic Tool)](snowcd) and [SYSTEM$ALLOWLIST\_PRIVATELINK](../sql-reference/functions/system_allowlist_privatelink).
6. Connect to Snowflake with your private connectivity account [URL](organizations-connect.html#label-connecting-via-url).

   Note that if you want to connect to Snowsight via Azure Private Link, follow the instructions in the
   [Snowsight documentation](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in).
...
* [Partner applications](oauth-snowflake-overview.html#label-oauth-private-connectivity)
...
[Controlling network traffic with network policies](network-policies).
