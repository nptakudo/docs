...
* [Amazon S3](tables-iceberg-configure-external-volume-s3)
* [Google Cloud Storage](tables-iceberg-configure-external-volume-gcs)
* [Azure Storage](tables-iceberg-configure-external-volume-azure)
* [S3-compatible storage](tables-iceberg-s3-compatible)

Each external volume is associated with a particular [Active storage location](tables-iceberg-storage.html#label-tables-iceberg-ext-vol-active-storage-location),
...
To configure these directories for Snowflake-managed tables, see [Data and metadata directories](tables-iceberg-storage.html#label-tables-iceberg-configure-external-volume-base-location).
...
you can set the [EXTERNAL\_VOLUME](../sql-reference/parameters.html#label-external-volume) parameter at the following levels:
...
:   Users can execute the appropriate [CREATE <object>](../sql-reference/sql/create) or [ALTER <object>](../sql-reference/sql/alter) command
    to override the [EXTERNAL\_VOLUME](../sql-reference/parameters.html#label-external-volume) parameter value at the database or schema level.
...
* [Amazon S3](tables-iceberg-configure-external-volume-s3.html#label-configure-external-volume-snowsight)
* [Google Cloud Storage](tables-iceberg-configure-external-volume-gcs.html#label-configure-external-volume-gcs-create-snowsight)
* [Azure Storage](tables-iceberg-configure-external-volume-azure.html#label-configure-external-volume-azure-create-snowsight)
* [S3-compatible storage](tables-iceberg-s3-compatible.html#label-tables-iceberg-s3-compatible-snowsight)
...
see [External volume privileges](security-access-control-privileges.html#label-external-volume-privileges).
...
2. Switch to a role that has OWNERSHIP privileges on the external volume that you want to grant USAGE privileges to.
   ...
   For instructions on switching a role, see [Switch your primary role](ui-snowsight-gs.html#label-switching-your-active-role). For more information on the OWNERSHIP privilege,
   see [External volume privileges](security-access-control-privileges.html#label-external-volume-privileges).
...
7. Select your cloud storage provider and specify the configuration for the storage location that you’re adding:
   ...
   2. Specify the configuration for the storage location that you’re adding:
      ...
      | Region type | Specifies the cloud storage provider that stores your data files.  * Standard (default): S3 storage in public AWS regions outside of China. * Government (GovCloud): S3 storage in AWS [government regions](intro-regions.html#label-us-gov-regions). |
      | S3 role ARN | Specifies the case-sensitive Amazon Resource Name (ARN) of the AWS identity and access management (IAM) role that grants privileges on the S3 bucket containing your data files.  You recorded this value when you [created an IAM role](tables-iceberg-configure-external-volume-s3.html#label-configure-external-volume-s3-create-iam-role). |
   ...
   2. Specify the configuration for the storage location that you’re adding:
      ...
      | Use PrivateLink endpoint | Specifies whether to use outbound private connectivity to harden your security posture. For information about using outbound private connectivity, see [Private connectivity to external volumes for Microsoft Azure](tables-iceberg-configure-external-volume-azure-private). |
