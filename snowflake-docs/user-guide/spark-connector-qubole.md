...
For more information, see [Preparing an External Location For Files](spark-connector-install.html#label-preparing-external-location-for-spark).
...
3. Enter the required information in the following fields:
   ...
   * Host Address: Enter the base URL of your Snowflake account (e.g.
     `myorganization-myaccount.snowflakecomputing.com`). See [Configuring a client, driver, library, or third-party application to connect to Snowflake](gen-conn-config) for details on
