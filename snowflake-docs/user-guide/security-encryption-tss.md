...
safeguarding your key as mentioned in [Customer-managed keys](security-encryption-manage.html#label-customer-managed-keys). If the customer-managed key (CMK) in the composite master key
...
If you want to enable private connectivity for a CMK that is already activated with Tri-Secret Secure, see [Enable a private connectivity endpoint for an active CMK](security-encryption-tss-self-serve-private.html#label-tss-enable-pl-for-active-cmk)
