...
See also [Optimizing the warehouse cache](performance-query-warehouse-cache), which discusses how table data may be cached
...
By default, result reuse is enabled, but can be overridden at the account, user, and session level using the [USE\_CACHED\_RESULT](../sql-reference/parameters.html#label-use-cached-result) session parameter.
