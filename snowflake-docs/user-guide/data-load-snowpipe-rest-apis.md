...
>
>   This error code can also be returned if the role used when calling the endpoint does not have sufficient privileges. For more information, see [Grant access privileges](data-load-snowpipe-rest-gs.html#label-snowpipe-rest-endpoints-access-control).
...
* `account_identifier` (Required): Your unique Snowflake account identifier. The preferred format is `organization_name-account_name`. For alternative formats (account locator with region and cloud platform), see [Format 1 (preferred): Account name in your organization](admin-account-identifier.html#label-account-name).
