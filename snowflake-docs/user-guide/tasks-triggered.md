...
Use triggered tasks to run tasks whenever there’s a change in a [stream](streams-intro). This eliminates the need to poll a source frequently when the availability of new data is unpredictable. It also reduces latency because data is processed immediately.
...
* Directory tables. A directory table must be refreshed before a triggered task can detect the changes. To detect changes, you can perform either of the following tasks:
  ...
  + Set the [directory table to auto-refresh](data-load-dirtables-auto).
...
* Additional requirements based on [compute resources](tasks-intro.html#label-tasks-compute-resources):
...
> ![Diagram showing a serverless triggered task](../_images/tasks-serverless-triggered.svg)
...
For more information, see [serverless tasks](tasks-intro.html#label-tasks-compute-resources-serverless).
...
* If a task hasn’t run for 12 hours, Snowflake schedules a health check to prevent streams from becoming stale.
  ...
  For more information, see [Avoiding stream staleness](streams-manage.html#label-streams-manage-avoiding-stream-staleness).
* Triggered tasks run at most every 30 seconds by default. If a task gets triggered again while running, the next run starts 30 seconds after the previous one was scheduled. You can lower this interval to 10 seconds by setting the [USER\_TASK\_MINIMUM\_TRIGGER\_INTERVAL\_IN\_SECONDS](../sql-reference/parameters.html#label-user-task-minimum-trigger-interval-in-seconds) parameter.
* When a task is triggered by [Streams on views](streams-intro.html#label-streams-views), then any changes to tables referenced by the Streams on Views query will also trigger the task, regardless of any joins, aggregations, or filters in the query.

![A diagram shows how triggered tasks manage new data as it comes in, and also check for changes every 12 hours.](../_images/tasks-triggered.svg)
...
Example 3: Create a user-managed task that runs whenever data changes in a directory table. In the example, a stream — my\_directory\_table\_stream — is hosted on a [directory table](data-load-dirtables-manage) on a stage called my\_test\_stage.
