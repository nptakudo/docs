...
This topic describes how to configure access control security for [securable objects](security-access-control-overview.html#label-access-control-securable-objects) in your
account.
...
For more details, see [Access control best practices](security-access-control-considerations) and [Multi-factor authentication (MFA)](security-mfa).

## Creating custom roles

To follow the general principle of “least privilege”, we recommend creating custom roles that
[align with the business functions](security-access-control-considerations.html#label-aligning-object-access-with-business-functions) in your organization to permit SQL actions
...
You can also grant a global privilege to a role. See [Access control privileges](security-access-control-privileges) for the list of global privileges
available to grant to a role.

SQL:
:   Use the GRANT PRIVILEGE command. See [Privilege management](../sql-reference/commands-user-role.html#label-sql-privilege-management) for details.
...
:   1. Sign in to [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in).
    ...
    3. In the navigation menu, select Governance & security » Users & roles, and then select Roles.
    4. Select Table and locate and select the role that you created.
    5. In the role details page, select [![More options](../_images/snowsight-worksheet-explorer-ellipsis.png)](../_images/snowsight-worksheet-explorer-ellipsis.png) » Manage global privileges.
...
![Role hierarchy and privileges granted to each role](../_images/system-role-hierarchy-with-privileges.png)
...
[![Image showing the roles graph focused on the ORGADMIN role, with the ACCOUNTADMIN role appearing above it in the hierarchy, as well as a system role SNOWFLAKE$GDS_RL used for global data sharing.](../_images/snowsight-admin-roles-graph.png)](../_images/snowsight-admin-roles-graph.png)
...
When viewing the details for a role, you can select the [![More options](../_images/snowsight-worksheet-explorer-ellipsis.png)](../_images/snowsight-worksheet-explorer-ellipsis.png) to center the graph on the selected role or open the role detail
page.
...
For more specific information about granting privileges to users, see [Usage notes](../sql-reference/sql/grant-privilege-user.html#label-grant-privilege-user-usage-notes) for
...
information about the DISABLE\_USER\_PRIVILEGE\_GRANTS parameter, see [DISABLE\_USER\_PRIVILEGE\_GRANTS](../sql-reference/parameters.html#label-disable-user-privilege-grants).
...
Currently, [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in) only displays usage and billing information to account administrators. It is not possible to grant other
