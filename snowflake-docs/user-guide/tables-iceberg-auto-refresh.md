...
[Apache Iceberg™ tables](tables-iceberg). With automated refreshes, Snowflake polls your external Iceberg catalog in
...
and the [CATALOG\_TABLE\_NAME](../sql-reference/sql/create-iceberg-table-aws-glue.html#label-create-iceberg-table-aws-glue-table-name-param) from AWS Glue.
...
Snowflake records the event in the [event table for your account](../developer-guide/logging-tracing/event-table-setting-up.html#label-logging-event-table-set-for-account).
...
To capture automated refresh events, you must set the LOG\_LEVEL parameter at the Iceberg table level or account level.
The [LOG\_LEVEL](../sql-reference/parameters.html#label-log-level) determines which events to capture based on the following values:
...
* `STOPPED` means the automated refresh process encountered an unrecoverable error, and automated refreshes for the table have been
  ...
  2. Perform a manual metadata refresh. For instructions, see [Refresh the table metadata](tables-iceberg-manage.html#label-tables-iceberg-refresh-metadata).
...
For more information about Iceberg table charges, see [Iceberg table billing](tables-iceberg.html#label-tables-iceberg-billing).
...
* Automated refresh synchronizes schema changes alongside [DML](../sql-reference/sql-dml) operations such as INSERT, UPDATE,
  or DELETE. To apply schema changes made through DDL operations alone, perform a [manual refresh](tables-iceberg-manage.html#label-tables-iceberg-refresh-metadata).
