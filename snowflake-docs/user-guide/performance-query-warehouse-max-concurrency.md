...
resources. You can use the [MAX\_CONCURRENCY\_LEVEL](../sql-reference/parameters.html#label-max-concurrency-level) parameter to limit the number of concurrent queries
...
[Query Acceleration Service](performance-query-warehouse-qas) can boost the performance of a large or complex query without impacting
the rest of the workload.

For more information, see [MAX\_CONCURRENCY\_LEVEL](../sql-reference/parameters.html#label-max-concurrency-level).

Note

> Adjusting the [STATEMENT\_QUEUED\_TIMEOUT\_IN\_SECONDS](../sql-reference/parameters.html#label-statement-queued-timeout-in-seconds) parameter can cancel queries rather than let them remain in the queue for an extended period of time.
