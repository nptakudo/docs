...
url: "https://docs.snowflake.com/en/user-guide/data-load-snowpipe-snowsight"
---

# Manage Snowpipe in Snowsight

You can use Snowsight to view [Snowpipe](data-load-snowpipe-intro) details and perform some pipe management tasks.
...
To view details about the pipe, you must use a role with the MONITOR or OWNERSHIP privilege on the pipe and the USAGE privilege on both the database and schema that contain the pipe. For more information, see [Pipe privileges](security-access-control-privileges.html#label-access-control-privileges-pipe).
...
3. Locate the database and schema that contain the pipe.
   ...
   ![Screenshot of where to find the Pipe Details page in Snowsight](../_images/ui-snowpipe-intro.png)
4. Select the pipe to open the details.

![Screenshot of the Pipe Details page in Snowsight](../_images/ui-snowpipe-details.png)

The Pipe Details page includes information about the following:

* [Status](../sql-reference/functions/system_pipe_status.html#label-snowpipe-status). Examples: Running; Paused.
...
* The incoming [Notification channel](data-load-snowpipe-auto) to tell the Pipe when there are new files.
...
* The [Privileges](security-access-control-configure.html#label-snowsight-manage-object-privileges) granted on the pipe.

### Manage pipes

You can perform the following tasks from the Pipe Details page:

* To add a comment to the pipe, select [![More options](../_images/snowsight-worksheet-explorer-ellipsis.png)](../_images/snowsight-worksheet-explorer-ellipsis.png) » Edit. To edit other properties of a pipe, you must use the
  [CREATE PIPE](../sql-reference/sql/create-pipe) SQL command to replace the pipe.
* To pause or resume the pipe, select [![More options](../_images/snowsight-worksheet-explorer-ellipsis.png)](../_images/snowsight-worksheet-explorer-ellipsis.png) » Pause or Resume.
...
For more information about managing pipes, see [Managing Snowpipe](data-load-snowpipe-manage).

## Copy history

To view the copy history for any of your pipes, go to the Pipe Details page, and select the Copy History tab.

> ![Screenshot of the Copy History tab in Snowsight](../_images/ui-snowpipe-copy-history.png)
