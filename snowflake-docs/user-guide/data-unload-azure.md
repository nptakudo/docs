...
![Unloading data to Azure](../_images/data-unload-azure.png)
...
Follow the steps in [Allow the VNet subnet IDs](data-load-azure-allow) in the data loading configuration instructions.
...
account. For instructions, see [Configure an Azure container for loading data](data-load-azure-config). Note that we
...
The following example creates an external stage named `my_ext_unload_stage` with a container named `mycontainer` and a folder path named `unload`. The stage references the named file format object called `my_csv_unload_format` that was created in [File formats to unload data](data-unload-prepare):
