...
management and billing, [Replication and Failover/Failback](replication-intro), Snowflake Secure Data Sharing, and other
...
* Organization account: Special account used by organization administrators to manage multi-account organizations and to access usage data
  from premium views in the ORGANIZATION\_USAGE schema. For more information, see [Organization accounts](organization-accounts).
* Regular Snowflake account, including [trial accounts](admin-trial-account).
...
If you are the [organization administrator](organization-administrators), you can view the name of your organization and
...
See [Finding the organization and account name for an account](admin-account-identifier.html#label-account-name-find).
...
organization using the original [account URL](organizations-connect.html#label-connecting-via-url). If you keep the original account URL, it is automatically dropped
...
expire, see [Deleting an organization URL](organizations-manage-accounts-urls.html#label-drop-org-access-url).

## Deleting an organization

To delete your Snowflake organization:

1. [Delete all accounts in the organization](organizations-manage-accounts-delete), except the account being used for the
   deletion.
