...
constrain sensitive or private information (for example, name or phone number) when [sharing data securely](data-sharing-gs) between partners.
...
* For all of these reasons, if you need to prevent leakage about a specific column or entity, you should omit the column entirely from
  your data, or employ [differential privacy](diff-privacy/differential-privacy-overview).
...
:   In this example, `provider.account` is the [account identifier](admin-account-identifier.html#label-account-name) in the account name format:
...
This section describes how a projection policy interacts with a [masking policy](security-column-intro) and a
[row access policy](security-row-intro).
