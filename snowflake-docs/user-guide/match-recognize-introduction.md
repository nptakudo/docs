...
You cannot use the MATCH\_RECOGNIZE clause in a **recursive** [common table expression (CTE)](queries-cte).
...
[![Chart of the Stock Price for "ABCD"](../_images/match-recognize-stock-chart.png)](../_images/match-recognize-stock-chart.png)
...
[![Sequence of rows that match the "V" pattern](../_images/match-recognize-row-sequences.png)](../_images/match-recognize-row-sequences.png)
...
[![The number of price decreases and increases in each "V" pattern](../_images/match-recognize-stock-chart-increases-decreases.png)](../_images/match-recognize-stock-chart-increases-decreases.png)
...
Row patterns consist of *pattern variables*, [quantifiers](../sql-reference/constructs/match_recognize.html#label-match-recognize-quantifiers) (which are similar to those
used in regular expressions), and [operators](../sql-reference/constructs/match_recognize.html#label-match-recognize-operators). A pattern variable defines an expression
...
[navigational function](../sql-reference/constructs/match_recognize.html#label-match-recognize-navigational-functions) `LAG()` to specify price for the previous row.

The row pattern matches two sequences of rows, as illustrated below:

[![First sequence of rows that match the "V" pattern](../_images/match-recognize-first-pattern-match.png)](../_images/match-recognize-first-pattern-match.png)

[![Second sequence of rows that match the "V" pattern](../_images/match-recognize-second-pattern-match.png)](../_images/match-recognize-second-pattern-match.png)
...
As shown in the [illustration of matching sequences](#label-match-recognize-figure-overlapping-matches), a row can be part of
...
[the last row where the price increased](#label-match-recognize-figure-first-matching-sequence).
...
To partition the data and specify the order of rows, use the [PARTITION BY](../sql-reference/constructs/match_recognize.html#label-match-recognize-partition-by) and
[ORDER BY](../sql-reference/constructs/match_recognize.html#label-match-recognize-order-by) subclauses in `MATCH_RECOGNIZE`. For example:
...
[quantifier](../sql-reference/constructs/match_recognize.html#label-match-recognize-quantifiers), as you would in a
...
You can use [exclusion syntax](../sql-reference/constructs/match_recognize.html#label-match-recognize-exclusions) in the pattern to tell `MATCH_RECOGNIZE` to
...
[![3-day sliding window frame in 7-day window](../_images/window-functions-sliding-frame.png)](../_images/window-functions-sliding-frame.png)

You have already seen [navigational functions](../sql-reference/constructs/match_recognize.html#label-match-recognize-navigational-functions) such as `LAG()`
...
attempt to reference rows outside the match group (i.e. outside the [window frame](../sql-reference/functions-window-syntax.html#label-window-frames)).
...
[navigational function](../sql-reference/constructs/match_recognize.html#label-match-recognize-navigational-functions).
...
[exclusion](../sql-reference/constructs/match_recognize.html#label-match-recognize-exclusions) syntax (`{-` and `-}`).
