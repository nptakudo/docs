---
title: "Monitor credit usage with budgets"
...
[supported objects](budgets/custom-budget.html#label-budgets-supported-objects).

For both types of budgets, you must set up the spending limit and specify how you want to receive notifications.

To start using budgets in Snowflake, [activate the account budget](budgets/account-budget.html#label-activate-account-budget).
...
* To trigger actions when a spending threshold is met, see [Custom actions for budgets](budgets/custom-actions).
* To trigger actions when the budget cycle restarts, see [Cycle-start actions for budgets](budgets/cycle-start-actions).
...
To determine the current compute cost of your budgets, see [Exploring compute costs](budgets/cost.html#label-budgets-cost-compute).
...
[Access control for cost management](cost-access-control).
...
* [Create a custom role to manage the account budget](budgets/account-budget.html#label-account-budget-admin-role)
* [Create a custom role to monitor the account budget](budgets/monitor.html#label-account-budget-monitor-role)
* [Create a custom role to monitor a custom budget](budgets/monitor.html#label-custom-budgets-monitor-role)
* [Create a custom role to manage a custom budget](budgets/custom-budget.html#label-custom-budgets-admin-role)
* [Create a custom role to create budgets](budgets/custom-budget.html#label-custom-budgets-owner-role)
...
* The following account-level parameters must be unset or set to the default value:
  ...
  + [AUTOCOMMIT](../sql-reference/parameters.html#label-autocommit) must be unset or set to TRUE.

    If this parameter is set to FALSE, activating a budget might fail, or usage might not be tracked correctly.
  + [TIMESTAMP\_INPUT\_FORMAT](../sql-reference/parameters.html#label-timestamp-input-format) and [DATE\_INPUT\_FORMAT](../sql-reference/parameters.html#label-date-input-format) must be unset or set to AUTO.

    If these parameters aren’t set to AUTO, usage might not be tracked correctly.

  Before you activate budgets, [check the values of these parameters](../sql-reference/parameters.html#label-parameters-list-by-name) by executing the
