...
url: "https://docs.snowflake.com/en/user-guide/ui-snowsight-profile"
---
...
For more information, see [Snowsight and MFA](ui-snowsight-gs.html#label-ui-snowsight-gs-mfa-notifications).

## View notifications from Trust Center

1. Sign in to [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in).
2. In the lower-left corner, next to your name and role, select [![Bell icon](../_images/bell-icon.svg)](../_images/bell-icon.svg).
3. In Notifications, select a notification listed on either Unread or All.
   ...
   [Remediate security risks](trust-center/using-the-trust-center.html#label-trust-center-resolve-violations).
...
Your new password must comply with the password policy. See [Snowflake-provided password policy](password-authentication.html#label-snowflake-password-policy).

## Enroll in multi-factor authentication (MFA)

[MFA](security-mfa) provides increased login security for users connecting to Snowflake.
...
to authenticate. For more information, see [Recovering a user who is locked out](security-mfa.html#label-mfa-recovery).
...
See [Generating a programmatic access token](programmatic-access-tokens.html#label-pat-generate).
