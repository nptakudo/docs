...
After configuring your driver, you can evaluate and troubleshoot your network connectivity to Snowflake using [SnowCD](snowcd).
...
2. Create a DataFrame with the [autopushdown](#label-spark-options-autopushdown) option set to `off`. For example:
...
2. Specify `SNOWFLAKE_SOURCE_NAME` using the `format()` method. For the definition, see [Specifying the Data Source Class Name](#label-spark-data-source) (in this topic).
...
* There is an upper limit to the size of an individual row. For more details, see [Limits on Query Text Size](query-size-limits).
...
* Use only the TIMESTAMP\_LTZ data type in Snowflake.
  ...
  The default timestamp data type mapping is TIMESTAMP\_NTZ (no time zone), so you must explicitly set the [TIMESTAMP\_TYPE\_MAPPING](../sql-reference/parameters.html#label-timestamp-type-mapping) parameter to use TIMESTAMP\_LTZ.
...
* The time zone in Snowflake is set to `Europe/Warsaw`, which can happen by either:
  ...
  + Setting `sfTimezone` to `snowflake` for the connector and setting the [TIMEZONE](../sql-reference/parameters.html#label-timezone) session
...
* `<account_identifier>`: Your [account identifier](gen-conn-config).
...
For more information about configuring the `spark-shell` script, see [Step 4: Configure the Local Spark Cluster or Amazon EMR-hosted Spark Environment](spark-connector-install.html#label-spark-cluster-setup).
...
:   Specifies the *hostname* for your account in the following format:
    ...
    `account_identifier` is your [account identifier](gen-conn-config).
...
* `pem_private_key`
  ...
  Private key (in PEM format) for key pair authentication. For instructions, see [Key-pair authentication and key-pair rotation](key-pair-auth).
* `sfAuthenticator`

  Specifies using [External OAuth](oauth-ext-overview) to authenticate to Snowflake. Set the value to `oauth`.
...
:   This parameter allows the user to specify the format for `TIME` data returned.
    ...
    The possible values of this parameter are the possible values for time formats specified at [Time formats](../sql-reference/date-time-input-output.html#label-time-formats).
...
:   This parameter is used when the query result set is very large and needs to be split into multiple DataFrame
    ...
    This option applies only when the [use\_copy\_unload](#label-use-copy-unload) parameter is FALSE.
...
1. To start, complete the initial configuration for key pair authentication as shown in [Key-pair authentication and key-pair rotation](key-pair-auth).
...
Starting with Spark Connector version 2.7.0, you can use [External OAuth](oauth-ext-overview) to authenticate to Snowflake using either the sample Scala program or the sample Python script.

Before using External OAuth and the Spark Connector to authenticate to Snowflake, configure an External OAuth security integration for one of the supported External OAuth authorization servers or an External OAuth [custom client](oauth-ext-custom).
...
:   If set to `on` (default), the connector checks if the bucket used for data transfer has a lifecycle policy configured (see [Preparing an AWS External S3 Bucket](spark-connector-install.html#label-spark-s3-setup) for more information). If there is no lifecycle
...
The Snowflake Connector for Spark supports sending arbitrary session-level parameters to Snowflake (see [Session parameters](../sql-reference/parameters.html#label-session-parameters) for more info). This can be achieved by adding a
...
For example, the following code sample passes the [USE\_CACHED\_RESULT](../sql-reference/parameters.html#label-use-cached-result) session parameter with a value of `"false"`, which disables using the results of previously-executed queries:
