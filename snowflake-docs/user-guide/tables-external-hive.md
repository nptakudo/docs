...
4. In a text editor, open the `snowflake-config.xml` file, and then populate the file with the following `<name>` properties and corresponding `<values>`:
   ...
   >     to a user, see [Key-pair authentication and key-pair rotation](key-pair-auth).
   >
   ...
   >     >     :   Names of your Snowflake organization and account. For information, see [Format 1 (preferred): Account name in your organization](admin-account-identifier.html#label-account-name).
   >     >
   >     >     Alternatively, specify your *account locator* and the geographical [region](intro-regions), and possibly the [cloud platform](intro-cloud-platforms), where the account is hosted. For more information, see [Format 2: Account locator in a region](admin-account-identifier.html#label-account-locator).
   >
