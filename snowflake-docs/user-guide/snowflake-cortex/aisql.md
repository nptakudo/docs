...
Your users need both the USE AI FUNCTIONS account-level privilege and the CORTEX\_USER role to use all Snowflake Cortex AI Functions.
...
Your users need **both** the USE AI FUNCTIONS account-level privilege and the CORTEX\_USER role to use ALL Snowflake Cortex AI Functions. If a user has access to the CORTEX\_USER role, but not the USE AI FUNCTIONS account-level privilege, they can still use the AI\_AGG and AI\_SUMMARIZE AGG functions.
...
* Qualified model object identifiers are quoted and therefore case-sensitive. For more information, see
  [QUOTED\_IDENTIFIERS\_IGNORE\_CASE](../../sql-reference/parameters.html#label-quoted-identifiers-ignore-case).
