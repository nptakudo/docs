...
![A two-dimensional example of vector similarity](../../_images/vector-similarity-vectors-example.png)
...
Supported models might have different [costs](aisql.html#label-cortex-llm-cost-considerations).
...
This SQL example calls the VECTOR\_COSINE\_SIMILARITY function to find the vector closes to `[1,2,3]`:
...
To create a vector embedding from a piece of text, you can use the [EMBED\_TEXT\_768 (SNOWFLAKE.CORTEX)](../../sql-reference/functions/embed_text-snowflake-cortex) or
[EMBED\_TEXT\_1024 (SNOWFLAKE.CORTEX)](../../sql-reference/functions/embed_text_1024-snowflake-cortex) functions, depending on the output dimensions of the model.
...
EMBED\_TEXT\_768 and EMBED\_TEXT\_1024 are Cortex LLM Functions, so their usage is governed by the same access controls as the other
Cortex LLM Functions. For instructions on accessing these functions, see the [Cortex LLM Functions Required Privileges](aisql.html#label-cortex-llm-privileges).
...
For more information about billing of Cortex LLM Functions, see [Cortex LLM Functions Cost Considerations](aisql.html#label-cortex-llm-cost-considerations).
For general information about compute costs, see [Understanding compute cost](../cost-understanding-compute).
