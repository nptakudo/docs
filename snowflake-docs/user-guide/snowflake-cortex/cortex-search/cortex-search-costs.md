...
url: "https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-search/cortex-search-costs"
---

# Understanding cost for Cortex Search Services

Support for this feature is available to accounts in most Snowflake regions.
See the [Regional availability](cortex-search-overview.html#label-cortex-search-overview-regional-availability) section for a full list of regions.
...
:   Defining primary keys on your Cortex Search Service can result in significant reductions to both the cost and latency
    ...
    the previous week. For more information on defining primary keys, see [Primary keys](cortex-search-overview.html#label-cortex-search-primary-keys).
...
:   There is a fixed component to the cost of an update, so fewer, bigger updates are less expensive than more frequent,
    ...
    to a row into a single update. For more information about vector embedding costs, see [Vector Embeddings](../vector-embeddings).
...
:   Any change to the schema of the source query causes a full refresh of the service, including vector embeddings and
    ...
    refresh and embed all vectors again. It’s better to update the source table incrementally (for example, with MERGE INTO). For more information about vector embedding costs, see [Vector Embeddings](../vector-embeddings).
...
:   A running search service incurs costs even if it is not serving queries. [Suspend the service](../../../sql-reference/sql/alter-cortex-search)
...
cost of each embedding model, see [Cortex Search Embedding Models](cortex-search-overview.html#label-cortex-search-embedding-models):
...
of each embedding model, see [Cortex Search Embedding Models](cortex-search-overview.html#label-cortex-search-embedding-models):
...
The [virtual warehouse](../../cost-understanding-compute.html#label-virtual-warehouse-credit-usage) compute cost for Cortex Search Services can vary based on the change rate of your data, target lag, and warehouse size.
...
Cortex Search Services use [Cloud Services compute](../../cost-understanding-compute.html#label-cloud-services-credit-usage) to trigger refreshes when an underlying base object has changed. These costs
