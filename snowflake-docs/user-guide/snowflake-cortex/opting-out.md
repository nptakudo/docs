...
the SNOWFLAKE.COPILOT\_USER database role, also granted to PUBLIC by default.) Two features, Cortex Analyst and Document
...
* **Cortex Fine-tuning:** Revoke the CREATE MODEL privilege on schemas from any roles you have granted it to.
  ...
  Scroll to top
* **Document AI:** Revoke the SNOWFLAKE.DOCUMENT\_INTELLIGENCE\_CREATOR database role from any
...
| --- | --- | --- | --- |
| [Cortex Agents](cortex-agents.html#label-cortex-agents-access-control) |  | SNOWFLAKE.CORTEX\_USER database role | USAGE on the search service that the agent queries, plus USAGE on the database, schema, and table used by the search service |
| [Cortex AI Functions](aisql.html#label-cortex-llm-privileges) |  | SNOWFLAKE.CORTEX\_USER database role |  |
| [Cortex Analyst](cortex-analyst.html#label-analyst-access-control) | ✔ | ENABLE\_CORTEX\_ANALYST account parameter |  |
| [Cortex Fine-tuning](cortex-finetuning.html#label-cortex-finetune-privileges) | ✔ | CREATE MODEL on the schema where you create fine-tuned models |  |
| [Cortex Knowledge Extensions](cortex-knowledge-extensions/cke-overview) |  | SNOWFLAKE.CORTEX\_USER database role | Relies on access control for the underlying Cortex Search Service |
| [Cortex Provisioned Throughput](provisioned-throughput) | ✔ | CREATE PROVISIONED THROUGHPUT privilege on the schema where you create provisioned throughput objects |  |
| [Cortex Search](cortex-search/cortex-search-overview.html#label-cortex-search-privileges) |  | SNOWFLAKE.CORTEX\_USER database role | USAGE on the search service, database, schema, and table used by the search service |
| [Document AI](document-ai/setting-up.html#label-document-ai-access-control) | ✔ | SNOWFLAKE.DOCUMENT\_INTELLIGENCE\_CREATOR database role | Numerous object-level privileges for creating models and pipelines |
| [Snowflake Copilot](../snowflake-copilot.html#label-snowflake-copilot-limit) |  | SNOWFLAKE.COPILOT\_USER database role |  |
| [Snowflake Intelligence](snowflake-intelligence) |  | SNOWFLAKE.CORTEX\_USER database role | Relies on access control for the underlying Cortex Agent or Search Service |
...
[Control model access](aisql.html#label-cortex-llm-access-control).
...
[Create alerts on new data](../alerts.html#label-alerts-type-streaming) in these views to notify you when new AI features are
