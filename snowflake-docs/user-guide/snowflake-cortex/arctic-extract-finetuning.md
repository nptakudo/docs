...
url: "https://docs.snowflake.com/en/user-guide/snowflake-cortex/arctic-extract-finetuning"
---
...
You can now fine-tune `arctic-extract` models using the [Snowflake Cortex Fine-tuning](cortex-finetuning)
...
role to the user who will call the function. See [LLM Functions required privileges](aisql.html#label-cortex-llm-privileges)
topic for details.
...
| `file3.pdf` | Expand  ``` {   "schema":   {     "type": "object",     "properties": {       "deductions": {         "description": "Deductions",         "type": "object",         "properties": {           "deductions_name": {             "type": "array"           },           "current": {             "type": "array"           }         }       }     }   } } ```  Show lessSee more  Scroll to top | Expand  ``` {   "deductions": {     "deductions_name": [       "Federal Tax",       "Wyoming State Tax",       "SDI",       "Soc Sec / OASDI",       "Health Insurance Tax",       "None"     ],     "current": [       "82.50",       "64.08",       "None",       "13.32",       "91.74",       "21.46"     ]   } } ```  Show lessSee more  Scroll to top |
...
* A limit exists on how many questions and documents can be in a fine-tuning job. Number of questions multiplied by total number
  ...
  | Number of questions | Number of pages | Number of document file references [[1]](#id3) |
  | --- | --- | --- |
  | 10 | 1 | 5,000 |
  | 100 | 1 | 500 |
  | 10 | 10 | 500 |
...
For more information, see [Copy arctic-extract models between databases, schemas, and accounts](copy-arctic-extract-models).
