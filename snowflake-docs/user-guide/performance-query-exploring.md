...
This topic explains how to examine the past performance of queries and [tasks](tasks-intro). This information helps
...
The [Query Profile](ui-snowsight-activity.html#label-snowsight-query-profile) allows you to examine which parts of a query are taking the longest to execute.
...
other users to access these views, refer to [Enabling other roles to use schemas in the SNOWFLAKE database](../sql-reference/account-usage.html#label-enabling-usage-for-other-roles).
...
Query Performance:
:   * [Query: Top n longest-running queries](#label-perf-explore-query-longest-run)
...
Warehouse Load:
:   * [Query: Total warehouse load](#label-perf-explore-query-warehouse-load)

Task Performance:
:   * [Query: Longest running tasks](#label-perf-explore-longest-running-tasks)
...
You can use the [query hash](query-hash) (the value of the `query_hash` column in the ACCOUNT\_USAGE
