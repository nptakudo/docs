...
![Line chart that shows sensor temperature readings dramatically increasing for a period of time.](../_images/time-series-data-spike.png)
...
sometimes also by sensor ID or a similar dimension). See [Clustering Keys & Clustered Tables](tables-clustering-keys).
...
[![Chart that shows a moving window frame for seven days with a missing record for Day 4.](../_images/range-between-time-series-gaps.png)](../_images/range-between-time-series-gaps.png)
...
To run this example, follow these instructions first: [Create and load the heavy\_weather table](../sql-reference/functions-window-syntax.html#label-create-heavy-weather-table).
...
![Line chart that shows a more jagged line over time for temperature and a smoother line for average temperature.](../_images/window-agg-smoothing.png)
...
![Trades table data, consisting of three rows, which are joined with three rows in the quotes table.](../_images/trades_table.png)

**QUOTES Table (Right or “Referenced” Table)**

![Quotes table data, consisting of seven rows, identifying the three specific rows that qualify for the join with the quotes table.](../_images/quotes_table.png)
...
For more examples of ASOF JOIN queries, see [Examples](../sql-reference/constructs/asof-join.html#label-asof-join-examples).
...
this kind of change, use the [RESAMPLE](../sql-reference/constructs/resample.html#label-resample-clause) clause, which you define within the FROM clause of a SELECT statement. The result of a
...
If you want to remove rows that don’t occur at uniform intervals (`09:49` and `10:18` in this case), see [RESAMPLE example that uses BUCKET\_START() to filter out non-uniform rows](../sql-reference/constructs/resample.html#label-resample-example-with-filter).
...
* The `bucket_start` metadata column returns the value that marks the beginning of the current bucket or interval
  ...
  use it to run aggregate queries on resampled data. See [RESAMPLE example that uses BUCKET\_START() to aggregate resampled rows](../sql-reference/constructs/resample.html#label-resample-examples-with-metadata-cols).

For the complete RESAMPLE syntax, see [RESAMPLE](../sql-reference/constructs/resample.html#label-resample-clause).

To store the results of a RESAMPLE query, use a [CTAS statement](../sql-reference/sql/create-table.html#label-ctas-syntax) that selects and inserts the data into a new table:
...
For more information about these window functions, see [INTERPOLATE\_BFILL, INTERPOLATE\_FFILL, INTERPOLATE\_LINEAR](../sql-reference/functions/interpolate_bfill.html#label-interpolation-functions).
...
* [Time-Series Forecasting](ml-functions/forecasting)
* [Anomaly Detection](ml-functions/anomaly-detection)
* [Top Insights](ml-functions/top-insights)
...
When the model has built successfully, call the [<model\_name>!DETECT\_ANOMALIES](../sql-reference/classes/anomaly-detection/methods/detect_anomalies.html#label-class-anomaly-detection-detect-anomalies)
