...
url: "https://docs.snowflake.com/en/user-guide/governance-skills"
---
...
1. [Install Cortex Code CLI and connect to your account](cortex-code/cortex-code-cli).
...
* [Privileges and roles required by Cortex Code](cortex-code/cortex-code-cli.html#label-cortex-code-cli-prerequisites).
...
* Access to views in the ACCOUNT\_USAGE schema. By default, only the ACCOUNTADMIN system role has privileges to access the views in the ACCOUNT\_USAGE schema. To
  ...
  + Grant database roles needed to access views. To use Cortex Code for all governance-related topics, a user needs all of these database roles:
    ...
    governance, grant a subset of these roles. For a list of the views that each role can access, see [ACCOUNT\_USAGE schema](../sql-reference/snowflake-db-roles.html#label-db-roles-account-usage).
