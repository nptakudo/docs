...
For the complete syntax and parameter descriptions, see [CREATE STAGE](../sql-reference/sql/create-stage). To add a directory table to an existing stage, use the [ALTER STAGE … SET DIRECTORY](../sql-reference/sql/alter-stage.html#label-alter-stage-directory-table-syntax) command.
...
To configure automated refreshes, see [Automated directory table metadata refreshes](data-load-dirtables-auto).
...
* Manual refreshes perform a list operation on a stage, and can be slow or expensive for large or fast-growing stages.
  Instead, use event-based [automated refreshes](data-load-dirtables-auto).
...
see [best practices for staging your data files](data-load-considerations-stage).
