...
url: "https://docs.snowflake.com/en/user-guide/tables-iceberg-load"
---
...
* [Snowpipe Streaming Classic with Apache Iceberg™ tables](snowpipe-streaming/snowpipe-streaming-classic-iceberg)
...
* An external volume named `iceberg_ingest_vol`. To create
  an external volume, see [Configure an external volume](tables-iceberg-configure-external-volume).
...
2. Create a Snowflake-managed Iceberg table, defining columns with data types that are compatible with the source Parquet file data types:
   ...
   see [Data types for Apache Iceberg™ tables](tables-iceberg-data-types).
