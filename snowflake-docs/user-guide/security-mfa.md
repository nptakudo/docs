...
[Configuring a second factor of authentication](security-mfa-second-factor).
...
information about these user types, see [Types of users](admin-user-management.html#label-user-management-types).
...
information about this rollout, see [Planning for the deprecation of single-factor password sign-ins](security-mfa-rollout).
...
* If an account existed before the 2024\_08 bundle was enabled, then you must configure your account if you want to require all human users to
  ...
  [Hardening user or account authentication using MFA](authentication-policies.html#label-authentication-policy-hardening-authentication).
* If the account was created after the 2024\_08 bundle was enabled, then all human users who authenticate with a password must enroll in MFA
  ...
  Be aware that the ability to opt out of mandatory MFA for human users is temporary; see [Planning for the deprecation of single-factor password sign-ins](security-mfa-rollout).
...
[authentication policy](authentication-policies) to require SSO users to use Snowflake MFA after authenticating with the
IdP.
...
As an administrator, you can use an [authentication policy](authentication-policies) to control which MFA methods can be
...
* If the user has a [verified email](ui-snowsight-profile.html#label-snowsight-verify-email-address), Snowflake sends an email prompting them to add an MFA
...
1. The [ALLOW\_CLIENT\_MFA\_CACHING](../sql-reference/parameters.html#label-allow-client-mfa-caching) parameter is set to FALSE for the account.
...
[single sign-on](admin-security-fed-auth-use.html#label-browser-based-sso-connection-caching). The client application stores the MFA token in the keystore of the
...
MFA token caching can be combined with connection caching in federated [single sign-on](admin-security-fed-auth-use.html#label-browser-based-sso-connection-caching).

To combine these two features, ensure that the [ALLOW\_ID\_TOKEN](../sql-reference/parameters.html#label-allow-id-token) parameter is set to `true` in tandem with the ALLOW\_CLIENT\_MFA\_CACHING parameter.
...
3. Add the package or libraries needed by the driver or connector:
   ...
   * If you are using the Snowflake Connector for Python, install the optional keyring package by running:
     ...
     >
     > If you need to install other extras (for example, `pandas` for [using the Python Connector APIs for Pandas](../developer-guide/python-connector/python-connector-pandas.html#label-python-connector-pandas-install)), use a comma between the extras:
     >
     > CopyExpand
     >
     > ```
...
For more details, see [Use multi-factor authentication (MFA)](../developer-guide/snowflake-cli/connecting/configure-connections.html#label-snowcli-mfa).
...
For more details, see [Use an MFA passcode](../developer-guide/node-js/nodejs-driver-authenticate.html#label-nodejs-mfa).
...
For more details, see the description of the connect() method in the [Functions](../developer-guide/python-connector/python-connector-api.html#label-snowflake-connector-methods) section of the Python
