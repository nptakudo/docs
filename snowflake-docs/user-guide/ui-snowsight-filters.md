...
3. Select [![Show or hide filter](../_images/snowsight-query-history-filter.png)](../_images/snowsight-query-history-filter.png) and, if in a worksheet, select Manage Filters.
...
To review custom filters in your account, open a worksheet or dashboard and then select [![Show or hide filter](../_images/snowsight-query-history-filter.png)](../_images/snowsight-query-history-filter.png).
...
See [Monitor query activity with Query History](ui-snowsight-activity) for details on using Query History.
...
* For a custom filter set to use a value type of text, the column must be a text string or cast to a text string in the query.
  See [Data types for text strings](../sql-reference/data-types-text.html#label-character-datatypes).
...
* `:daterange`
  ...
  + Filters a column by a date range, such as Last day, Last 7 days, Last 28 days, Last 3 months,
    ...
    The date range filter always uses the UTC time zone and is not affected by the [TIMESTAMP\_INPUT\_FORMAT](../sql-reference/parameters.html#label-timestamp-input-format) parameter.
...
[![When the filter buttons appear, they follow the show and hide filters button in the tab order.](../_images/snowsight-query-filter-example.png)](../_images/snowsight-query-filter-example.png)
