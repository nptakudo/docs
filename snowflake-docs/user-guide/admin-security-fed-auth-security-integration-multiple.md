...
2. [Configure each identity provider](admin-security-fed-auth-configure-idp).
3. [Create multiple SAML security integrations](admin-security-fed-auth-security-integration.html#label-saml-sso-create-integration), one for each IdP.
...
[ENABLE\_ICEBERG\_MERGE\_ON\_READ](../sql-reference/parameters.html#label-enable-identifier-first-login) parameter to `TRUE`:
...
For more information about the identifier-first login flow, see [Identifier-first login](identifier-first-login).
...
:   Use the `SECURITY_INTEGRATIONS` property of an [authentication policy](authentication-policies) to specify which
    ...
    For an example of using an authentication policy to implement multiple IdPs, see [Allow authentication from multiple identity providers on an account](authentication-policies.html#label-authentication-policy-multiple-idp-example).
...
3. Sign in to [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in).
4. In the navigation menu, select Projects » Worksheets.
5. Switch to a role with the [CREATE INTEGRATION](../sql-reference/sql/create-security-integration-saml2.html#label-create-security-integration-saml2-access-control-requirements) privilege.
