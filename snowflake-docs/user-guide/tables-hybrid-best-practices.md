...
url: "https://docs.snowflake.com/en/user-guide/tables-hybrid-best-practices"
---
...
[Snowsight](ui-snowsight) provides rich access to query plans, data statistics, query history,
...
Run a [simple performance test](tables-hybrid-test) to validate performance for
your scenario.
...
* Understand the cardinality of your data before creating indexes. Indexes built with a single, low-cardinality column have
  limited benefit. See [Estimating the Number of Distinct Values](querying-approximate-cardinality).
...
latency. See [INCLUDE columns](tables-hybrid-index.html#label-indexes-with-include-columns).
...
* Use [CREATE TABLE … AS SELECT (also referred to as CTAS)](../sql-reference/sql/create-table.html#label-ctas-syntax) for creating and immediately loading empty tables.
...
[multi-cluster warehouse](warehouses-multicluster) rather than
...
`Standard` rather than `Economy` for the best performance. see [Multi-cluster warehouses](warehouses-multicluster).

## Stored procedures and hybrid tables

Stored procedures are supported for hybrid tables; however, executing
transactions with [AUTOCOMMIT](../sql-reference/parameters.html#label-autocommit) enabled or multi-statement transactions
...
For more information, see [Quotas and throttling](tables-hybrid-limitations.html#label-hybrid-tables-limitations-ht-quotas-throttling).
...
* Workload characteristics:
  ...
  + Column [cardinality estimates](querying-approximate-cardinality)
