...
* By path (internal stages) / prefix (Amazon S3 bucket). See [Organizing data by path](data-load-considerations-stage.html#label-organizing-data-by-path) for information.
...
This section describes how the [COPY INTO <table>](../sql-reference/sql/copy-into-table) command prevents data duplication differently based on whether the load status for a file is known or unknown. If you partition your data in stages using logical, granular paths by date (as recommended in [Organizing data by path](data-load-considerations-stage.html#label-organizing-data-by-path)) and load data within a short period of time after staging it, this section largely does not apply to you. However, if the COPY command skips older files (i.e. historical data files) in a data load, this section describes how to bypass the default behavior.
...
![../_images/data-load-status1.svg](../_images/data-load-status1.svg)
...
![../_images/data-load-status2.svg](../_images/data-load-status2.svg)
