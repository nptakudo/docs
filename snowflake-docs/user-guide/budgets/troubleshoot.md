...
url: "https://docs.snowflake.com/en/user-guide/budgets/troubleshoot"
---
...
[ENABLE\_BUDGET\_EVENT\_LOGGING](../../sql-reference/parameters.html#label-enable-budget-event-logging) account parameter to `FALSE` to turn it off.
...
|  |  |
| --- | --- |
| Error | Expand  ``` Unknown user-defined function SNOWFLAKE.LOCAL.ACTIVATE ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` FAILURE: Uncaught exception of type 'BUDGET_ALREADY_ACTIVATED' on line X at position X ```  Show lessSee more  Scroll to top |
| Cause | The account budget has already been activated. |
| Solution | You can call the [<budget\_name>!GET\_CONFIG](../../sql-reference/classes/budget/methods/get_config) method to view the activation timestamp:  CopyExpand  ``` CALL SNOWFLAKE.LOCAL.ACCOUNT_ROOT_BUDGET!GET_CONFIG(); ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` -20000 (P0001): Uncaught exception of type 'NO_PERMISSION_TO_ACTIVATE_BUDGET' on line X at position X ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` FAILURE: SQL access control error: Insufficient privileges to operate on class 'BUDGET' ```  Show lessSee more  Scroll to top |
| Cause | The role you are using does not have the privileges required to create custom budgets. |
| Solution | Use a role with the required privileges. See [Create a custom role to create budgets](custom-budget.html#label-custom-budgets-owner-role). |
...
|  |  |
| --- | --- |
| Error | Expand  ``` FAILURE: Uncaught exception of type 'STATEMENT_ERROR' on line 0 at position -1 : Uncaught exception of type 'UNSUPPORTED_BUDGET_TYPE' on line X at position X ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` -20000 (P0001): Uncaught exception of type 'FUNCTION_NOT_SUPPORTED_FOR_ACCOUNT_ROOT_BUDGET' on line 11 at position 18 ```  Show lessSee more  Scroll to top |
...
| Solution | These methods are not available on the account budget. The account budget monitors all supported objects in the account and objects cannot be added or removed. For more information, see [Account budget and custom budgets](../budgets.html#label-account-and-custom-budgets). |
...
|  |  |
| --- | --- |
| Error | Expand  ``` -20000 (P0001): Uncaught exception of type 'ACCOUNT_ROOT_BUDGET_NOT_ACTIVATED' on line X at position X ```  Show lessSee more  Scroll to top |
| Cause | You tried to call a method on the account budget before the account budget is activated. |
| Solution | [Activate the account budget](account-budget.html#label-activate-account-budget). |
...
[required privileges and role](monitor.html#label-custom-budgets-monitor-role).

|  |  |
| --- | --- |
| Error | Expand  ``` 002141 (42601): SQL compilation error: Unknown user-defined function <budget_db>.<budget_schema>.<budget_name>!ADD_RESOURCE ```  Show lessSee more  Scroll to top |
...
| Solution | Grant the required instance role and privileges to the role used to call the method. For more information, see [Create a custom role to monitor a custom budget](monitor.html#label-custom-budgets-monitor-role). |
...
|  |  |
| --- | --- |
| Error | Expand  ``` 002003 (02000): SQL compilation error: <object_type> '<object_name>' does not exist or not authorized. ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` Uncaught exception of type 'EXPRESSION_ERROR' on line 10 at position 21 : Privilege 'APPLYBUDGET' is not authorized on the reference object. ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` 505001 (55000): Uncaught exception of type 'EXPRESSION_ERROR' on line 10 at position 21 : Specified object does not exist or not authorized for the reference. ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` Unknown user-defined function <database_name>.<schema_name>.<budget_name>.SET_EMAIL_NOTIFICATIONS ```  Show lessSee more  Scroll to top |
| Cause | The role you used to set the email notifications for a custom budget does not have the ADMIN instance role. |
| Solution | Use a role with the required privileges and roles. See the [Access control requirements](../../sql-reference/classes/budget/methods/set_email_notifications.html#label-set-email-notifications-acr) for SET\_EMAIL\_NOTIFICATIONS. |
...
|  |  |
| --- | --- |
| Error | Expand  ``` Integration '<INTEG_NAME>' does not exist or not authorized. ```  Show lessSee more  Scroll to top |
| Cause | The notification integration does not exist. |
| Solution | Use a valid notification integration. For more information, see [Create an email notification integration](../notifications/email-notifications.html#label-create-email-notification-integration). Include the email addresses for budgets notifications in the ALLOWED\_RECIPIENTS list. |
...
|  |  |
| --- | --- |
| Error | Expand  ``` FAILURE: Uncaught exception of type 'EXPRESSION_ERROR' on line 16 at position 34 : Following email address(es) are not allowed by the email integration <INTEGRATION_NAME>: [<email>] ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` Email recipients in the given list at indexes [<index_list>] are not allowed. Either these email addresses are not yet validated or do not belong to any user in the current account. ```  Show lessSee more  Scroll to top |
| Cause | Some or all of the email addresses you tried to add are not validated. |
| Solution | See [Verify the email addresses of the email notification recipients](../notifications/email-notifications.html#label-email-notification-verify-address). |
...
|  |  |
| --- | --- |
| Error | Expand  ``` 001044 (42P13): SQL compilation error: error line 0 at position -1 Invalid argument types for function 'GET_SERVICE_TYPE_USAGE': (VARCHAR(X), VARCHAR(X), VARCHAR(X), VARCHAR(X)) ```  Show lessSee more  Scroll to top |
...
|  |  |
| --- | --- |
| Error | Expand  ``` 002151 (22023): Uncaught exception of type 'STATEMENT_ERROR' on line 16 at position 23 : SQL compilation error: [:TIME_DEPART] is not a valid date/time component for function DATE_TRUNC. ```  Show lessSee more  Scroll to top |
| Cause | The TIME\_DEPART argument is an invalid string. |
| Solution | Use one of the valid values listed for the [TIME\_DEPART argument](../../sql-reference/classes/budget/methods/get_service_type_usage.html#label-get-service-type-usage-time-depart) in the reference topic. |
...
|  |  |
| --- | --- |
| Error | Expand  ``` 100094 (22000): Uncaught exception of type 'STATEMENT_ERROR' on line 16 at position 23 : Unknown timezone: '<invalid_timezone>' ```  Show lessSee more  Scroll to top |
| Cause | The USER\_TIMEZONE argument is an invalid string. |
| Solution | Use a valid timezone string. For more information, see the [usage notes for GET\_SERVICE\_TYPE\_USAGE](../../sql-reference/classes/budget/methods/get_service_type_usage.html#label-get-service-type-usage-usage-notes). |
...
|  |  |
| --- | --- |
| Errors | Expand  ``` FAILURE: SQL compilation error: Class 'SNOWFLAKE.CORE.BUDGET' does not exist or not authorized. ```  Show lessSee more  Scroll to top  Expand  ``` 000002 (0A000): Uncaught exception of type 'STATEMENT_ERROR' on line 0 at position -1 : Unsupported feature 'TOK_RESOURCE_GROUP'. ```  Show lessSee more  Scroll to top |
| Cause | Your account does not yet support the Budgets feature. |
| Solution | The Budgets feature is not available for accounts in Gov regions. Support for Gov regions will be available in a future release. |
