...
url: "https://docs.snowflake.com/en/user-guide/budgets/custom-budget"
---
...
[Tags](../object-tagging/introduction) can be applied to budgets to monitor credit usage by objects that belong to a logical
...
For more information, including how tag values are overridden, see [Tag inheritance](../object-tagging/inheritance).
...
| Compute pool | Compute pool usage for Snowpark Container Services. For more information, see [Compute pool cost](../../developer-guide/snowpark-container-services/accounts-orgs-usage-views.html#label-compute-pool-cost). |
...
| Materialized views | Background maintenance for the materialized view. For more information, see [Materialized Views Cost](../views-materialized.html#label-materialized-views-maintenance-billing). |
...
| Pipes | Resource consumption for loading data using Snowpipe. For more information, see [Snowpipe costs](../data-load-snowpipe-billing). |
| Tables | Background maintenance operations for [automatic clustering](../tables-auto-reclustering) and [search optimization](../search-optimization-service) if they are enabled on the table. |
| Tasks | Serverless tasks are monitored by a custom budget. To monitor the credit usage for a task that executes using a user-managed warehouse, you must add the warehouse to the budget. For more information, see [Task costs](../tasks-intro.html#label-billing-task-runs). |
| Warehouses | Compute resources for query execution, web interface, and other features (see [Virtual warehouse credit usage](../cost-understanding-compute.html#label-virtual-warehouse-credit-usage)), serverless tasks, and [cloud services compute](../cost-understanding-compute.html#label-cloud-services-credit-usage). |
...
role to create a custom budget, see [Budgets roles and privileges](../budgets.html#label-budgets-roles-and-privileges).
...
If the account budget is not [activated](account-budget.html#label-activate-account-budget) or has
...
5. On the Basic Information page, complete the following steps:
   ...
   4. To decrease the [budget refresh interval](../budgets.html#label-budgets-refresh-interval) so you can watch spending more closely, select
   ...
   6. In the Notify field, enter email addresses to receive notification emails.
      ...
      Each email address added for budget notifications must be [verified](../notifications/email-notifications.html#label-email-notification-verify-address). The
...
and usage. For more information, see [Create a custom role to monitor a custom budget](monitor.html#label-custom-budgets-monitor-role).
...
4. Set up notifications for the budget so that you receive notifications when your credit usage is expected to exceed your
   ...
   See [Notifications for budgets](notifications).
...
resources and usage. For more information, see [Create a custom role to monitor a custom budget](monitor.html#label-custom-budgets-monitor-role).
...
and roles that must be granted to a role to modify a custom budget, see [Budgets roles and privileges](../budgets.html#label-budgets-roles-and-privileges).
...
5. In the dashboard, select [![Pencil icon](../../_images/pencil-icon.svg)](../../_images/pencil-icon.svg) (edit icon).
6. Select + Tags & resources.
7. Expand Tags and navigate to the tag you want to add.
8. Do one of the following:

   * If the tag has a [list of allowed values](../object-tagging/work.html#label-object-tagging-specify-tag-values), select one or more of the values.
...
1. You can add table `t1` to budget `my_budget` by using the following steps:
   ...
   2. Pass a reference for table `t1` to the [ADD\_RESOURCE](../../sql-reference/classes/budget/methods/add_resource) instance
      ...
      For a full list of objects and privileges, see [Supported object types and privileges for references](../../sql-reference/references.html#label-references-supported-objects-and-privileges).
