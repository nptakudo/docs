...
url: "https://docs.snowflake.com/en/user-guide/data-quality-ui-monitor"
---
...
To gain a better understanding of data quality and DMFs, see [Introduction to data quality checks](data-quality-intro).
...
5. Do one of the following:
   ...
   * If you already have DMFs associated with the object, start exploring! You can only see a DMF if you have the appropriate
     [access control privileges](data-quality-access-control).
...
* System DMFs are grouped based on their [category](data-quality-system-dmfs.html#label-dmf-system). For example, the NULL\_COUNT and BLANK\_COUNT DMFs are grouped
...
* All [custom DMFs](data-quality-custom-dmfs) associated with the object are grouped under Custom.
...
DATA\_METRIC\_SCHEDULE parameter of the object. For more information, see [Adjust the schedule for DMFs](data-quality-working.html#label-data-quality-schedule).
...
see [Use SQL to work with expectations](data-quality-expectations).
...
:   To better understand the quality check that you’re investigating:
    ...
    2. In the Quality Checks section, check the Status column to determine which quality check failed. This corresponds to the
       [expectation](data-quality-expectations) that was violated.
    3. For each failed quality check, use the Expression column to determine the value that the quality check expected the DMF to return.
       This corresponds to the [expression of the expectation](data-quality-expectations.html#label-dmf-expectation-expression).
...
:   With the side panel open, find the Impacted Assets section so you can determine what other objects might be affected by the
    quality issue. For information about interpreting the list of objects, see [Impacted Assets section](#label-data-quality-ui-drill-impacted).

Step 6: Which records violated the quality check? ([Select system DMFs only](data-quality-fixing.html#label-data-quality-remediation-supported-dmfs))
:   1. With the side panel open, select View Failed Records.
    2. Execute the prepopulated query to see the records that failed the quality check. This query calls the SYSTEM$DATA\_METRIC\_SCAN
       ...
       [Using SYSTEM$DATA\_METRIC\_SCAN to fix data](data-quality-fixing.html#label-data-quality-remediate).
...
View Lineage button
:   Select a DMF to view the [lineage](ui-snowsight-lineage) of the object associated with that DMF.

View failed records button ([Select system DMFs only](data-quality-fixing.html#label-data-quality-remediation-supported-dmfs))
:   If the DMF returned a value greater than 0, you can determine which records were flagged as having quality issues. For example, if the
    ...
    For more information about using the SYSTEM$DATA\_METRIC\_SCAN function, see [Remediation of data quality issues](data-quality-fixing).
...
:   Lists the [expectations](data-quality-expectations) that were added to the association between the DMF and the
    ...
    * Expression — Expression of the expectation. For more information, see [Defining what meets the expectation](data-quality-expectations.html#label-dmf-expectation-expression).
    * Status — Indicates whether the expectation was violated the last time the DMF ran.

Impacted Assets section
:   Displays the objects that are [downstream](ui-snowsight-lineage.html#label-lineage-upstream-downstream) in the lineage of the object with which the DMF is
