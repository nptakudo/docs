...
* Your organization has an ORGADMIN role. ([Organization accounts](../../../organization-accounts) are optional.)
...
* You cannot specify specific regions in organizational listings using Snowsight.
  ...
  Instead, you can specify the region in the [manifest YAML](org-listing-manifest-reference)
  file when [creating](../../../../sql-reference/sql/create-organization-listing) or [altering](../../../../sql-reference/sql/alter-listing) the listing programatically.
...
| MODIFY with grants option | Can `ALTER` a share after granting modify privileges to a role. This can be done using:  CopyExpand  ``` grant modify on data exchange listing <listing_name> to role <role_name> ```  Show lessSee more  Scroll to top |
...
To directly consume an organizational listing, you can reference the [Uniform Listing Locator (ULL)](org-listing-configure.html#label-olisting-im-ull) without any additional privileges. If you require mounting the listing, then the following privileges are required:
...
A [role](../../../security-access-control-overview.html#label-access-control-overview-roles) used to execute this operation must have the following
[privileges](../../../security-access-control-overview.html#label-access-control-overview-privileges) at a minimum:
...
For a complete list of all fields and values for an organizational listing see [Organization listing manifest reference](org-listing-manifest-reference).
For additional examples, see [Set who can discover and access an organizational listing](org-listing-configure.html#label-organizational-listing-discovery-and-access).
