...
url: "https://docs.snowflake.com/en/user-guide/database-replication-considerations"
---
...
For additional guidance for working with replicated objects and data, refer to [Replication considerations](account-replication-considerations).
...
:   The replication operation fails if either of the following conditions is true:
    ...
    * An object contained in the primary database has a [dangling reference](#label-database-replication-dangling-references) to a tag
    ...
    For more information about tag-based masking policies, refer to [Tag-based masking policies](tag-based-masking-policies).
...
:   The replication operation fails if either of the following conditions is true:
    ...
    * Either of these objects contained in the primary database is attached to a user in the same account. In this case, Snowflake fails
    ...
    [replication or failover group](account-replication-intro.html#label-replication-and-failover-groups) instead.

    For details, refer to [Replication and security policies](account-replication-considerations.html#label-account-replication-considerations-security-policies).
...
:   You cannot replicate a secret using database replication. Use a replication or failover group to replicate a secret. For details, see
    [Replication and secrets](account-replication-considerations.html#label-account-replication-considerations-secrets).
...
| [Stream](account-replication-considerations.html#label-dangling-reference-streams) | Object | Fails |
...
* A materialized view references any object in another database.
  ...
  [replication or failover group](account-replication-intro.html#label-replication-and-failover-groups). Alternatively, you
...
* A materialized view is invalid (i.e. references a [dropped object](#label-references-to-dropped-objects)).
  ...
  to the [Troubleshooting](views-materialized.html#label-troubleshooting-materialized-views) section in the materialized views topic.
...
To work around this limitation, replicate both databases together in the same
[replication or failover group](account-replication-intro.html#label-replication-and-failover-groups). Alternatively, you can
...
To work around this limitation, replicate both databases together in the same
[replication or failover group](account-replication-intro.html#label-replication-and-failover-groups). Alternatively, you can reference sequences in the
same database.

#### Masking & row access policies and tags

A dangling reference for a [masking policy](security-column-intro.html#label-security-column-replication),
[row access policy](security-row-intro.html#label-security-row-replication), or [tag](object-tagging/interaction.html#label-tags-replication) causes the replication to fail with
...
![Simple diagram of successful refresh with different failover groups.](../_images/dynamic-tables-data-replication-example-1.png)
...
![Simple diagram of successful refresh with different failover groups.](../_images/dynamic-tables-data-replication-example-2.png)
