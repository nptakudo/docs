...
assigning a masking policy to a tag, see [Tag-based masking policies](tag-based-masking-policies).

Important

External tokenization requires [Writing external functions](../sql-reference/external-functions), which are included in the Snowflake [Standard Edition](intro-editions.html#label-snowflake-editions-standard), and you can use external functions with a tokenization provider.

However, if you choose to integrate your tokenization provider with Snowflake External Tokenization, you must upgrade to
[Enterprise Edition](intro-editions.html#label-snowflake-editions-enterprise) or higher.
...
For more details about how masking policies work, including the query runtime behavior, creating a policy, usage with tables and views, and management approaches using masking policies, see: [Understanding Column-level Security](security-column-intro).

For more details on the effects of the SQL execution context and role hierarchy, see [Advanced Column-level Security topics](security-column-advanced).
...
For a comparison of benefits between Dynamic Data Masking and External Tokenization, see: [Column-level Security Benefits](security-column-intro.html#label-security-column-benefits).

## External Tokenization limitations

For an overview on the limitations, see [Column-level Security Limitations](security-column-intro.html#label-security-column-limitations).

## External Tokenization considerations

For additional External Tokenization Considerations, see [Column-level Security Considerations](security-column-intro.html#label-security-column-considerations).
...
* The [MASKING POLICIES](../sql-reference/account-usage/masking_policies) view provides a list of all masking policies in your
  Snowflake account.
...
Snowflake records the original query run by the user on the [History page](ui-snowsight-activity) (in the web interface). The query
is found in the SQL Text column.

The masking policy names that were used in a specific query can be found in the [Query Profile](ui-snowsight-activity).
...
* [Using Conditional Tokenization](security-column-intro.html#label-security-column-intro-cond-cols)
