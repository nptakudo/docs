...
When an account is renamed, Snowflake creates a new [account URL](organizations-connect.html#label-connecting-via-url) that is used to access the account.
...
[deleted at a later time](organizations-manage-accounts-urls.html#label-drop-account-access-url).

Note

Renaming an account has no effect on [replication and failover](account-replication-intro).

[As the organization administrator](organization-administrators), you can rename an account using [Snowsight](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in)
