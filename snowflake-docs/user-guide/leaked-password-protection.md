...
1. If one or more [verified email addresses](notifications/email-notifications.html#label-email-notification-verify-address) for account-level security notifications is
...
For more information about notification contacts for Snowflake, see [Set up and manage notification contacts for Snowflake](ui-snowsight-contacts).

## Types of users scanned

All [user types](admin-user-management.html#label-user-management-types) are scanned, and Snowflake can disable any user’s password if the user has a password.

## Reset a user password

If Snowflake blocks a user from authenticating with a password, then the user must ask their administrator to
[reset their password](password-authentication.html#label-user-management-reset-password).
