...
You can use the NOTIFICATION\_HISTORY table function to query the history of notifications sent through Snowpipe. For more information, see [NOTIFICATION\_HISTORY](../sql-reference/functions/notification_history).
...
1. Create a topic to receive the notifications, and set up a notification integration for that topic.
   ...
   * [AWS SNS](notifications/creating-notification-integration-amazon-sns)
   * [Google Pub/Sub](notifications/creating-notification-integration-google-pubsub)
   * [Azure Event Grid](notifications/creating-notification-integration-azure-event-grid)
2. Create or configure the task to use the notification integration for error and success notifications.

   See [Configure a task to send error notifications](tasks-errors-integrate) and [Configure a task to send success notifications](tasks-success-integrate).
