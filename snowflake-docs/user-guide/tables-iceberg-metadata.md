...
To set the retention period for table data and snapshots, set the [DATA\_RETENTION\_TIME\_IN\_DAYS](../sql-reference/parameters.html#label-data-retention-time-in-days) parameter
...
For instructions, see [Generate snapshots of DML changes](tables-iceberg-manage.html#label-tables-iceberg-generate-snapshots).

For information about locating metadata files, see [Data and metadata directories](tables-iceberg-storage.html#label-tables-iceberg-configure-external-volume-base-location).
...
For viewing options, see [Monitor query activity with Query History](ui-snowsight-activity).
...
Snowflake does not support [Fail-safe](data-failsafe) for Snowflake-managed Iceberg tables,
...
For [converted tables](tables-iceberg-conversion),
...
For tables that use an external catalog, Snowflake uses the value of the [DATA\_RETENTION\_TIME\_IN\_DAYS](../sql-reference/parameters.html#label-data-retention-time-in-days)
...
* The following value, depending on your [Snowflake account edition](intro-editions):
...
`history.expire.max-snapshot-age-ms` in your metadata file and then [refresh the table](tables-iceberg-manage.html#label-tables-iceberg-refresh-metadata).
...
configure your external volume with write access (see [ALLOW\_WRITES](../sql-reference/sql/create-external-volume.html#label-create-external-volume-allow-writes-param)).
For more information about the write location, see [Data and metadata directories](tables-iceberg-storage.html#label-tables-iceberg-configure-external-volume-base-location).

To prevent Snowflake from writing Iceberg metadata, you can set the ALLOW\_WRITES parameter to FALSE on your
...
* Creating partitioned Iceberg tables
  that are Snowflake-managed or externally managed in a [catalog-linked database](tables-iceberg-catalog-linked-database)
  or [externally managed by an Iceberg REST catalog](tables-iceberg-externally-managed-writes).
...
For an example, see [Create an Iceberg table in a catalog-linked database](tables-iceberg-externally-managed-writes.html#label-tables-iceberg-external-writes-create-table-cld).
...
see [File management](tables-iceberg-storage.html#label-tables-iceberg-manage-files-in-storage).
...
see [Create an Iceberg table in a catalog-linked database with hierarchical path layout](tables-iceberg-externally-managed-writes.html#label-tables-iceberg-external-writes-create-table-cld-hierarchical).
...
* Support for version 3 of the Apache Iceberg™ specification is in public preview. This public preview includes support for using deletion
  vectors with partitioned tables. For more information about this public preview, see [Apache Iceberg™ tables: Support for Apache Iceberg™ v3 (Preview)](tables-iceberg-v3-specification-support).
...
| COPY INTO <table> | Limited support | Limited support | Limited support | Limited support | See [Usage notes](../sql-reference/sql/copy-into-table.html#label-copy-into-table-usage-notes). |
| CREATE ICEBERG TABLE … AS SELECT (CTAS) | ✔ | ✔ | ✔ | ✔ |  |
| Cloning | ✔ | ✔ | ✔ | ✔ | See usage notes:   * [Snowflake managed](../sql-reference/sql/create-iceberg-table-snowflake.html#label-create-iceberg-table-snowflake-usage-notes) * [Externally managed](../sql-reference/sql/create-iceberg-table-rest.html#label-create-catalog-table-rest-usage-notes) |
| CREATE ICEBERG TABLE … LIKE | ✔ | ✔ | ✔ | ✔ | See usage notes:   * [Snowflake managed](../sql-reference/sql/create-iceberg-table-snowflake.html#label-create-iceberg-table-snowflake-usage-notes) * [Externally managed](../sql-reference/sql/create-iceberg-table-rest.html#label-create-catalog-table-rest-usage-notes) |
...
| Partition transforms | ✔ | ✔ | ✔ | ✔ | For the supported partition transforms, see:   * [Snowflake managed](../sql-reference/sql/create-iceberg-table-snowflake.html#label-create-iceberg-table-snowflake-partitionexpressions) * [Externally managed](../sql-reference/sql/create-iceberg-table-rest.html#label-create-iceberg-table-rest-partitionexpressions) |
| Positional deletes | ✔ | ✔ | ✔ | ✔ |  |
| Snowpipe | Limited support | Limited support | Limited support | Limited support | * Currently in *Public Preview*. * See the [usage notes](../sql-reference/sql/copy-into-table.html#label-copy-into-table-usage-notes) for COPY INTO <table>. |
| Snowpipe Streaming | ❌ | ❌ | ❌ | ❌ |  |
...
* You can’t use the ALTER ICEBERG TABLE command to modify the PATH\_LAYOUT property for an existing table.
...
With [Snowflake Time Travel](data-time-travel),
...
queries on Snowflake-managed tables when you [Sync a Snowflake-managed table with Snowflake Open Catalog](tables-iceberg-open-catalog-sync) or
use the [Snowflake Catalog SDK](tables-iceberg-catalog).

You can query any snapshots that were committed within the data retention period.
To specify the data retention period, set the [DATA\_RETENTION\_TIME\_IN\_DAYS](../sql-reference/parameters.html#label-data-retention-time-in-days) object parameter.
