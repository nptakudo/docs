...
> [![The row access policy admin can apply row access policies to tables and views.](../_images/security-row-intro-admin-apply-policies.png)](../_images/security-row-intro-admin-apply-policies.png)
...
[![Row access policies with tables and views.](../_images/security-column-row-intro-policy-table-view.png)](../_images/security-column-row-intro-policy-table-view.png)
...
* [Use row access policies](security-row-using)
...
:   Policies with simple SQL expressions, such as CASE statements, generally perform better than policies that access mapping (i.e. lookup)
    ...
    * [Memoizable function](../developer-guide/udf/sql/udf-sql-scalar-functions.html#label-udf-sql-scalar-memoizable) (in the scalar SQL UDF overview).
    * [Using a memoizable function in a policy](security-row-using.html#label-security-row-using-example-memoizable-function)
...
:   For very large tables, clustering by attributes used for policy filtering can improve performance.
    ...
    For more information, see [Clustering Keys & Clustered Tables](tables-clustering-keys).
...
* [Future grants](../sql-reference/sql/grant-privilege.html#label-grant-privilege-schema-future-grants) of privileges on row access policies are not supported.
...
* Including one or more [subqueries](querying-subqueries) in the policy body may cause errors. When possible, limit the
...
When a database object has both a row access policy and one or more [masking policies](security-column-intro),
...
You can apply the row access policy to VALUE column of an existing external table by executing an [ALTER TABLE](../sql-reference/sql/alter-table)
...
In the EXPLAIN command result and the [Query History](ui-snowsight-activity) page, Snowflake does not show users any
...
For more information, see [Understanding & using Time Travel](data-time-travel).
...
* [Context functions and row access policies in Streamlit in Snowflake](../developer-guide/streamlit/features/row-access.html#label-streamlit-context-functions)
* [Example: Access data in a table with row access policy using CURRENT\_USER](../developer-guide/streamlit/features/row-access.html#label-streamlit-context-functions-example)
...
* Account-level query:
  ...
  Use the Account Usage [POLICY\_REFERENCES](../sql-reference/account-usage/tag_references) view to determine all of the tables
...
* Row access policy administrators (i.e. users with the row access policy [OWNERSHIP privilege](#label-security-row-privileges)) can
  use [Time Travel](data-time-travel) or [streams](streams-intro) to capture historical data about any
