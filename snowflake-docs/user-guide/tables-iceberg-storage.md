...
For more information, see [Use catalog-vended credentials for Apache Iceberg™ tables](tables-iceberg-configure-catalog-integration-vended-credentials).
...
Each external volume is associated with a particular [Active storage location](#label-tables-iceberg-ext-vol-active-storage-location),
...
To configure these directories for Snowflake-managed tables, see [Data and metadata directories](#label-tables-iceberg-configure-external-volume-base-location).
...
Snowflake provisions a principal for your entire Snowflake account when you create an [external volume](tables-iceberg.html#label-tables-iceberg-external-volume-def).
...
* [Configure an external volume for Amazon S3](tables-iceberg-configure-external-volume-s3)
...
To grant Snowflake access to an [S3-compatible storage location](data-load-s3-compatible-storage) for Iceberg tables,
you specify an [S3-compatible storage endpoint](data-load-s3-compatible-storage.html#label-data-load-s3-compatible-storage-reqs) with credentials when you create an external volume.

For instructions, see [Configure an external volume for S3-compatible storage](tables-iceberg-s3-compatible).
...
To verify storage access by using Snowsight, see [Verify an external volume by using Snowsight](tables-iceberg-configure-external-volume.html#label-tables-iceberg-configure-external-volume-verify-snowsight)
...
metadata for [Delta-based tables](tables-iceberg-metadata.html#label-tables-iceberg-metadata-delta).

Snowflake constructs paths using the following patterns, depending on the values specified
for [BASE\_LOCATION](../sql-reference/sql/create-iceberg-table-snowflake.html#label-create-iceberg-table-snowflake-base-location) or
the [BASE\_LOCATION\_PREFIX](../sql-reference/parameters.html#label-base-location-prefix) parameter.
...
table. For instructions on how to enable this layout, see [Partitioning with hierarchical paths](tables-iceberg-metadata.html#label-tables-iceberg-partitioning-hierarchical-paths). Snowflake writes all
...
Additionally, [converting a table](tables-iceberg-conversion) does not rewrite any data or metadata files.
...
how to enable this layout, see [Partitioning with hierarchical paths](tables-iceberg-metadata.html#label-tables-iceberg-partitioning-hierarchical-paths). Snowflake writes all table metadata data files
...
For more information, see [Partitioning with hierarchical paths](tables-iceberg-metadata.html#label-tables-iceberg-partitioning-hierarchical-paths).
...
For more information, see [Configure replication for Snowflake-managed Apache Iceberg™ tables](tables-iceberg-replication).
