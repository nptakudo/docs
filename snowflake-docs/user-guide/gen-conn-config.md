...
url: "https://docs.snowflake.com/en/user-guide/gen-conn-config"
---
...
1. [Sign in](ui-snowsight-gs.html#label-snowsight-getting-started-sign-in) to Snowsight.
2. Open the user menu by selecting your user name.

   [![Account menu](../_images/snowsight-gs-user-menu.png)](../_images/snowsight-gs-user-menu.png)
3. From the user menu, select Connect a tool to Snowflake to display the Account Details dialog.

   Tip

   You can also display the account details from the [account selector](ui-snowsight-gs.html#label-snowsight-account-details).
4. Select one of the following tabs:
   ...
   * If your client, driver, library, or third-party application supports using a TOML configuration file (for example,
     ...
     [Snowflake Python APIs](../developer-guide/snowflake-python-api/snowflake-python-connecting-snowflake.html#label-snowflake-python-connect-config-file), or the
     [Snowflake Connector for Python](../developer-guide/python-connector/python-connector-connect.html#label-python-connection-toml):
     ...
     4. From the Connection Method menu, select the method that you plan to use to authenticate:
        ...
        + To use [browser-based single sign-on (SSO)](admin-security-fed-auth-use.html#label-browser-based-sso), select Web Browser.
     ...
     5. Select the copy icon ([![For assistive technology, the copy button.](../_images/snowsight-query-copy.png)](../_images/snowsight-query-copy.png)) to copy the content for the configuration file.

     Note

     For the [Snowflake Python APIs](../developer-guide/snowflake-python-api/snowflake-python-connecting-snowflake.html#label-snowflake-python-connect-config-file), underscores are not supported in the
   ...
   * If your client, driver, library, or third-party application supports specifying a connection string (for example,
     the [ODBC Driver](../developer-guide/odbc/odbc-parameters.html#label-odbc-connection-string), [JDBC Driver](../developer-guide/jdbc/jdbc-configure.html#label-jdbc-connection-string),
     ...
     5. From the Connection Method menu, select the method that you plan to use to authenticate:
        ...
        Clients, drivers, libraries, and third-party applications support additional authentication methods not listed in
        the menu. For information, see [Securing Snowflake](../guides-overview-secure).
     6. Select the copy icon ([![For assistive technology, the copy button.](../_images/snowsight-query-copy.png)](../_images/snowsight-query-copy.png)) to copy the resulting connection string.
   * To execute SQL commands to get the configuration information:

     1. Select the SQL Commands tab.
     2. Select the copy icon ([![For assistive technology, the copy button.](../_images/snowsight-query-copy.png)](../_images/snowsight-query-copy.png)) next to the command that provides the information that you need, paste the
...
| Setting | SQL command |
| --- | --- |
| Account identifier for the current account | * To get the `organization_name-account_name` form of your account identifier:  CopyExpand  ```   SELECT CURRENT_ORGANIZATION_NAME() || '-' || CURRENT_ACCOUNT_NAME();   ```  Show lessSee more  Scroll to top * To get the [account locator](admin-account-identifier.html#label-account-locator) form of your account identifier:  CopyExpand  ```   SELECT CURRENT_ACCOUNT();   ```  Show lessSee more  Scroll to top |
| Current user name | CopyExpand  ``` SELECT CURRENT_USER(); ```  Show lessSee more  Scroll to top |
| Current role | CopyExpand  ``` SELECT CURRENT_ROLE(); ```  Show lessSee more  Scroll to top |
| Current region | CopyExpand  ``` SELECT CURRENT_REGION(); ```  Show lessSee more  Scroll to top |
| Current warehouse | CopyExpand  ``` SELECT CURRENT_WAREHOUSE(); ```  Show lessSee more  Scroll to top |
| Current database | CopyExpand  ``` SELECT CURRENT_DATABASE(); ```  Show lessSee more  Scroll to top |
| Current schema | CopyExpand  ``` SELECT CURRENT_SCHEMA(); ```  Show lessSee more  Scroll to top |
...
One exception to this rule is when you’re using the [Client Redirect](client-redirect) feature. If you’re
using this feature, replace the name of the account (`account_name`) with the name of the connection
(`connection_name`). For examples of this syntax, see [Using a connection URL](client-redirect.html#label-using-a-connection-url).
...
compliance as additional segments after the account locator. For the format to use, see [Format 2: Account locator in a region](admin-account-identifier.html#label-account-locator). In the
...
:   * Account name: `snowsql -a orgname-account_name`
    * Account locator: `snowsql -a account_locator_with_additional_segments`
    ...
    For additional information, see [Connection syntax](snowsql-start.html#label-connection-syntax).
...
:   * Account name: `jdbc:snowflake://orgname>-<account_name.snowflakecomputing.com/?connection_paramsr`
    * Account locator: `jdbc:snowflake://account_locator_with_additional_segments.snowflakecomputing.com/?connection_params`
    ...
    For additional information, see [JDBC Driver connection string](../developer-guide/jdbc/jdbc-configure.html#label-jdbc-connection-string).
...
:   * Account name: Same as JDBC
    * Account locator: Same as JDBC
    ...
    For additional information, see [Setting Configuration Options for the Connector](spark-connector-use.html#label-spark-options).

Spark (Databricks):
:   * Account name: `Account URL for Snowflake account`
    * Account locator: `Account Locator URL for Snowflake account`

    For additional information, see [Configuring Snowflake for Spark in Databricks](spark-connector-databricks).
...
:   * Account name: Set the Host Address field value as `orgname-account_name.snowflakecomputing.com`.
    * Account locator: Set the Host Address field value as
    ...
    For additional information, see [Configuring Snowflake for Spark in Qubole](spark-connector-qubole).
...
* [Allowing Host names](hostname-allowlist)
