...
url: "https://docs.snowflake.com/en/user-guide/diff-privacy/differential-privacy-privacy-domains"
---
...
Within [differential privacy in Snowflake](differential-privacy-overview), a *privacy domain* defines the possible
...
The privacy domain is one factor that Snowflake uses to calculate the amount of [noise](differential-privacy-overview.html#label-diff-privacy-noisy-aggregates) that
...
* An administrator uses CREATE TABLE and ALTER TABLE commands to set a privacy domain for a column. An administrator for the data provider
  sets privacy domains before giving access to analysts. [In some circumstances](differential-privacy-analyst.html#label-diff-privacy-joins-data-types), an
  ...
  you’re an administrator who needs to set privacy domains, see [Working with privacy domains as an administrator](differential-privacy-privacy-domains-admin).
* An analyst shapes a query to implicitly specify a privacy domain using query elements like filters and column transformations. These
  ...
  an analyst who needs to specify or narrow a privacy domain, see [Working with privacy domains as an analyst](differential-privacy-privacy-domains-analyst).
...
:   Snowflake treats numeric, date, and time values that fall outside the range of a privacy domain differently depending on
    ...
    :   When an analyst specifies a privacy domain for a column that doesn’t have one or narrows an admin-specified privacy domain, the
        ...
        * If the query uses a filter ([WHERE clause](differential-privacy-privacy-domains-analyst.html#label-diff-privacy-analyst-privacy-domain-set-range-where)), values outside of
          the privacy domain are ignored when calculating aggregations.
        * If the query uses a [column transformation](differential-privacy-privacy-domains-analyst.html#label-diff-privacy-analyst-privacy-domain-set-range-transformation), values in
