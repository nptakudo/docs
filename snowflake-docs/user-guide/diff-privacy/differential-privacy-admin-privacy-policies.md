...
[differential privacy](differential-privacy-overview).

## About privacy policies

With differential privacy, Snowflake must check each query to determine whether it will exceed the
[privacy budget](differential-privacy-overview.html#label-diff-privacy-loss-budgets) associated with the user executing the query. Privacy policies make that possible.
...
* If the policy associates the user with a privacy budget, Snowflake makes sure that the
  [privacy loss](differential-privacy-overview.html#label-diff-privacy-loss-budgets) incurred by the query does not exceed that privacy budget. If the query is executed
...
* If the policy indicates that the user can query the table without restriction, then the results don’t contain
  [noise](differential-privacy-overview.html#label-diff-privacy-noisy-aggregates), and Snowflake does not track the privacy loss incurred by the query.
...
* `column_name` specifies the entity key for the table or view. The [entity key](differential-privacy-admin.html#label-diff-privacy-admin-entity-privacy) is a
...
to protect individual rows without considering whether data belonging to an entity could exist in multiple rows. For more information, see [About entity-level privacy](differential-privacy-admin.html#label-diff-privacy-admin-entity-privacy).
