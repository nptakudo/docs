...
* If you want to decrease the noise, narrow the privacy domain to exclude or clamp values outside a useful range. For information about how
  values outside the privacy domain are treated, see [Values outside a privacy domain](differential-privacy-privacy-domains.html#label-diff-privacy-privacy-domains-excluded-values).
...
Remember that cumulative privacy loss is reset to 0 on a fixed schedule, as defined by the [budget window](differential-privacy-admin-privacy-budgets.html#label-diff-privacy-admin-privacy-budget-refresh-definition). When the privacy budget is reset, the analyst can run a fresh set of
...
function to see the effects of queries on the privacy budget, see [Tracking privacy budget spending](differential-privacy-analyst.html#label-diff-privacy-analyst-budget).
