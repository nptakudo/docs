...
This security feature currently requires that your S3 bucket is located in the same AWS [region](intro-regions) as your Snowflake account.
...
2. Execute [USE ROLE](../sql-reference/sql/use-role) to set ACCOUNTADMIN as the active role for the user session.
   ...
   Scroll to top
3. Query the [SYSTEM$GET\_SNOWFLAKE\_PLATFORM\_INFO](../sql-reference/functions/system_get_snowflake_platform_info) function to retrieve the IDs of the AWS Virtual Network (VNet) in which your Snowflake account is located:
