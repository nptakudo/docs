...
* To set up data quality checks for your data, see [Use Snowsight to set up data quality checks](data-quality-ui-setup).
* To monitor the results of your existing data quality checks, see [Monitoring data quality checks in Snowsight](data-quality-ui-monitor).
...
:   A DMF measures an attribute of your data such as how many NULL values exist in a column or how often a table is being updated. The DMF
    ...
    available for various dimensions, see [System data metric functions](data-quality-system-dmfs).
    ...
    see [Custom data metric functions](data-quality-custom-dmfs).
...
:   An expectation is combined with a DMF to create a data quality check. When a DMF returns a value, it’s compared to the expectation’s
    ...
    If you [use Snowsight to create a data quality check](data-quality-ui-setup), you choose the DMF and define the expectation at the
    same time. You can also [use SQL to work with expectations directly](data-quality-expectations).
...
:   Anomaly detection uses historical data to automatically detect when a DMF return value is above or below a predicted range. Currently,
    ...
    [Detecting anomalies in data quality](data-quality-anomaly).
...
:   The DMF schedule for a table or view determines how often a DMF runs. Because a DMF powers a data quality check, the DMF schedule
    ...
    a table or view, see [Adjust how often quality checks run](data-quality-ui-setup.html#label-data-quality-ui-setup-schedule).
...
The DMFs that power data quality checks use [serverless compute resources](cost-understanding-compute.html#label-serverless-credit-usage) that incur costs. For the
...
For information about replication and DMFs, see [Replication of data metric functions (DMFs)](account-replication-considerations.html#label-replication-data-quality).
...
* You can only have 10,000 total associations of DMFs on objects per account. Each instance of setting a DMF on a table or view counts as
  one association.
* [Data sharing](data-sharing-intro): You can’t grant privileges on a DMF to a share or set a DMF on a shared table or view.
* Setting a DMF on an object tag is not supported.
* You can’t set a DMF on objects in a [reader account](data-sharing-intro.html#label-about-reader-accounts).
