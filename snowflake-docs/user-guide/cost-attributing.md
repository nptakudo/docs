...
* Use [query tags](../sql-reference/parameters.html#label-query-tag) to associate individual queries with departments or projects when the queries are
...
* **Resources used exclusively by a single cost center or department:** An example of this is using object tags to associate
  ...
  [![Resources used exclusively by a single cost center or department](../_images/cost-attribute-non-shared.png)](../_images/cost-attribute-non-shared.png)
* **Resources that are shared by users from multiple departments:** An example of this is a warehouse shared by users from
  ...
  [![Resources that are shared by users from multiple departments](../_images/cost-attribute-user-level-share.png)](../_images/cost-attribute-user-level-share.png)
* **Applications or workflows shared by users from different departments:** An example of this is an application that issues
  ...
  [![Applications or workflows shared by users from different departments](../_images/cost-attribute-app-level-share.png)](../_images/cost-attribute-app-level-share.png)
...
* If the resources used by the department span multiple accounts, you [create the tags](#label-cost-attribute-tag-create)
  in a key account in your organization (for example, in your [organization account](organization-accounts)),
...
[privileges for object tagging](object-tagging/work.html#label-object-tags-ddl-privilege-summary) to develop a secure tagging strategy.
...
[set up your accounts for replication](account-replication-config), and
[create a replication group](../sql-reference/sql/create-replication-group) in a main account (for example, in the
[organization account](organization-accounts)). Set up this replication group to replicate the database
containing the tags.

[![Replicating the database containing the object tags](../_images/cost-attribute-replication.png)](../_images/cost-attribute-replication.png)
...
![Joining TAG_REFERENCES and WAREHOUSE_METERING_HISTORY to attribute costs for dedicated warehouses](../_images/cost-attribution-join-single-bu.png)
...
![Joining TAG_REFERENCES and WAREHOUSE_METERING_HISTORY to attribute costs for shared warehouses](../_images/cost-attribution-join-shared-bu.png)
...
[query acceleration service](query-acceleration-service) for the finance department. This depends on the
...
1. Switch to a role that has [access to the ACCOUNT\_USAGE schema](../sql-reference/account-usage.html#label-enabling-usage-for-other-roles).
...
For more details about filtering in Snowsight, see [Filter by tag](cost-exploring-compute.html#label-cost-exploring-filter-tag).
...
For an additional query based on query ID, see [Examples](../sql-reference/account-usage/query_attribution_history.html#label-query-attribution-history-examples).
...
1. To find the root query ID for a stored procedure, use the [ACCESS\_HISTORY view](../sql-reference/account-usage/access_history). For example,
   ...
   For more information, see [Ancestor queries with stored procedures](access-history.html#label-access-history-ancestor-queries).
