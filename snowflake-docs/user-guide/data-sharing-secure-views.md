...
[secure views](views-secure) and/or [secure UDFs](../developer-guide/secure-udf-procedure) instead of directly
sharing tables.

In addition, for optimal performance, especially when sharing data in extremely large tables, we recommend defining
[clustering keys](tables-clustering-keys.html#label-clustering-keys) on the base table(s) for your secure objects.
...
[important considerations](tables-clustering-keys.html#label-clustering-keys-strategies).
...
[SIMULATED\_DATA\_SHARING\_CONSUMER](../sql-reference/parameters.html#label-simulated-data-sharing-consumer). Set this session parameter to the name of the consumer account you wish to simulate
...
1. Create a [share](data-sharing-provider.html#label-share-create).
...
2. Add the database (`mydb`), schema (`public`), and secure view (`paid_sensitive_data`) to the share. You can choose
   ...
   share. For more information on these options, see [How to share database objects](data-sharing-gs.html#label-data-sharing-share-options).
