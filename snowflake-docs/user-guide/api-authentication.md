...
[RBAC](security-access-control-overview), and is encrypted using the Snowflake
[key encryption hierarchy](security-encryption-manage.html#label-encryption-key-mgmt). Information present in the secret object is encrypted using a key in the key
...
Additionally, you can replicate secrets using [account replication](account-replication-intro). For details, see
the Replication and secrets section in [Replication considerations](account-replication-considerations).
