...
improve performance by producing [ARRAYs](../sql-reference/data-types-semistructured.html#label-data-type-array) that contain the distinct values and computing the number
...
For other techniques for counting distinct values, see [Computing the Number of Distinct Values](querying-distinct-counts).
...
| --- | --- |
| CopyExpand  ``` SELECT   COUNT(DISTINCT my_column_1),   COUNT(DISTINCT my_column_2) FROM my_table; ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT   ARRAY_SIZE(ARRAY_UNIQUE_AGG(my_column_1)),   ARRAY_SIZE(ARRAY_UNIQUE_AGG(my_column_2)) FROM my_table; ```  Show lessSee more  Scroll to top |
...
| Example With COUNT(DISTINCT <expression>) | Example With ARRAY\_UNIQUE\_AGG(<expression>) |
| --- | --- |
| CopyExpand  ``` SELECT   COUNT(DISTINCT my_column_1),   COUNT(DISTINCT my_column_2) FROM my_table GROUP BY my_key_1, my_key_2; ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT   ARRAY_SIZE(ARRAY_UNIQUE_AGG(my_column_1)),   ARRAY_SIZE(ARRAY_UNIQUE_AGG(my_column_2)) FROM my_table GROUP BY my_key_1, my_key_2; ```  Show lessSee more  Scroll to top |
...
| Example With COUNT(DISTINCT <expression>) | Example With ARRAY\_UNIQUE\_AGG(<expression>) |
| --- | --- |
| CopyExpand  ``` SELECT   COUNT(DISTINCT my_column) FROM my_table GROUP BY ROLLUP(my_key_1, my_key_2); ```  Show lessSee more  Scroll to top | CopyExpand  ``` SELECT   ARRAY_SIZE(ARRAY_UNIQUE_AGG(my_column)) FROM my_table GROUP BY ROLLUP(my_key_1, my_key_2); ```  Show lessSee more  Scroll to top |
...
In these cases, consider using a [bitmap aggregation](querying-bitmaps-for-distinct-counts) instead. As an alternative, you
