...
url: "https://docs.snowflake.com/en/user-guide/object-tagging/introduction"
---
...
how best to make that data available, such as selective filtering using [row access policies](../security-row-intro), or
using [masking policies](../security-column-intro) to determine whether the data is tokenized, fully masked, partially
masked, or unmasked.
...
For an example of using tags to monitor resource usage, see [Setting up object tags for cost attribution](../cost-attributing.html#label-cost-attribute-tag).

## How a tag gets associated with an object

A tag can be associated with an object in the following ways:

* Someone manually set the tag on the object using a CREATE <object> or ALTER <object> command. See [Set a tag](work.html#label-object-tagging-set).
* The object inherited the tag from an object higher up in the Snowflake securable object hierarchy. For example, a warehouse in an account
  inherits tags set on the account. See [Tag inheritance](inheritance).
* The tag was automatically propagated from one object to another. Tags can be propagated when an object depends on another object (for
  ...
  to create a table). See [Automatic tag propagation with user-defined tags](propagation).
* The tag was automatically set on a column that was classified as containing sensitive data. To learn how sensitive data
  classification uses a tag map to set these tags, see [About classification tags](../classify-intro.html#label-classify-classification-tags).
...
|  | Alert |  |
|  | Backup set | For [WORM backups](../backups). Contains a set of backups for a specific database, schema, or table. |
...
Future grants:
:   [Future grants](../../sql-reference/sql/grant-privilege.html#label-grant-privilege-schema-future-grants) of privileges on tags are not supported.
