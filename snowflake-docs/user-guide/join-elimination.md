...
As mentioned in [Supported Constraint Types](../sql-reference/constraints-overview.html#label-constraints-supported-types), Snowflake does not enforce UNIQUE, PRIMARY KEY, and FOREIGN KEY
constraints on standard tables, but does enforce them on [hybrid tables](tables-hybrid). For standard tables, you are
