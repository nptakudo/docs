...
you understand how to read query plans that access hybrid tables. See also [Monitor query activity with Query History](ui-snowsight-activity).
...
* COLUMN-BASED: The query reads from an object storage copy of the same data that was loaded into the row store. Index scans can also access
  object storage, for [Time Travel](data-time-travel) queries.
...
Where possible, table scans for hybrid tables read data from a columnar warehouse cache. This cache is an extension to the standard warehouse cache; see [Optimizing the warehouse cache](performance-query-warehouse-cache). The cache contains data that has been read from the hybrid table storage provider and is
...
Queries that select from hybrid tables do not benefit from the [query results cache](querying-persisted-results).
...
[Quotas and throttling](tables-hybrid-limitations.html#label-hybrid-tables-limitations-ht-quotas-throttling).

[![Query profile overview shows a high throttling percentage for hybrid table requests.](../_images/hybrid_table_profile_overview_throttling.png)](../_images/hybrid_table_profile_overview_throttling.png)
...
[![TableScan operator for query that filters on the primary key column, timestamp](../_images/table_scan_pk_row_based.png)](../_images/table_scan_pk_row_based.png)
...
[![IndexScan operator with attributes, including access predicate and ROW_BASED scan mode](../_images/secondary_index_scan_with_access_predicate.png)](../_images/secondary_index_scan_with_access_predicate.png)

See also [INCLUDE columns](tables-hybrid-index.html#label-indexes-with-include-columns).
...
[![Table scan operator that uses a ROW_BASED scan for a single-row UPDATE](../_images/single_row_update_table_scan.png)](../_images/single_row_update_table_scan.png)
...
[![Table scan operator that read 100% of the data from the cache](../_images/table_scan_cached_100_percent.png)](../_images/table_scan_cached_100_percent.png)
...
[![TableScan operator for a hybrid table in a join, including access predicate and ROW_BASED scan mode](../_images/table_scan_join_hybrid_table.png)](../_images/table_scan_join_hybrid_table.png)
